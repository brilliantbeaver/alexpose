...
The target encoder receives the complete sequence. The view encoder receives the unmasked tokens after a small sequence-wide rotation and translation. If $\mathbf{z}_t$ is a target feature, $\mathbf{c}$ is a running center, and $\mathbf{p}$ is the prediction, the loss is

$$
\mathcal{L}=-\sum_d
\operatorname{softmax}\left(\frac{\mathbf{z}_t-\mathbf{c}}{0.06}\right)_d
\log \operatorname{softmax}\left(\frac{\mathbf{p}}{0.10}\right)_d .
$$
