...
def sample_joint_angles():
    ...
    mock_sequence = Mock()
    mock_sequence.get_statistics = Mock(
        side_effect=lambda joint: {
            "left_hip": {
                "mean": 175.0,
                "std": 5.0,
                "min": 165.0,
...
def sample_feature_vectors():
    ...
    for i in range(5):
        fv = GaitFeatureVector(
            ...
            right_ankle_range=30.0,  # Reduced
...
class TestGaitFeatureVector:
    ...
    def test_feature_vector_creation(self):
        ...
        fv = GaitFeatureVector(
            left_hip_mean=175.0, left_knee_mean=170.0, condition_label="normal"
        )
    ...
    def test_from_joint_angles(self, sample_joint_angles):
        ...
        fv = GaitFeatureVector.from_joint_angles(
            sample_joint_angles, sample_id="test_sample", condition_label="normal"
        )
        ...
        assert fv.left_ankle_range == 40.0
        assert fv.right_ankle_range == 40.0
...
class TestKNNGaitClassifier:
    ...
    def test_tune_hyperparameters(self, sample_feature_vectors):
        ...
        results = classifier.tune_hyperparameters(
            sample_feature_vectors, param_grid=param_grid
        )
