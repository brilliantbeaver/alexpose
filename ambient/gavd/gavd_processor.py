...
class GAVDDataLoader:
    ...
    def _download_youtube_videos(
        ...
        print("YouTube cache attempted: {}".format(summary["attempted"]))
        print("YouTube cache skipped: {}".format(summary["skipped"]))
        print("YouTube cache downloaded: {}".format(summary["downloaded"]))
        print("YouTube cache failed: {}".format(summary["failed"]))
            ...
            if self.estimator is not None and has_urls:
                ...
                try:
                    ...
                    if cache_key not in self._video_kp_cache:
                        ...
                        try:
                            print(f"\t...video_keypoints: {video_path}::{model_pose}", flush=True)
                            all_frames = self.estimator.estimate_video_keypoints(
                                video_path, model=model_pose
                            )
                        except AttributeError:
                            # Estimator doesn't support video mode; fall back per-frame
                            all_frames = None
                        self._video_kp_cache[cache_key] = all_frames
                    all_frames = self._video_kp_cache.get(cache_key)

                    if isinstance(all_frames, list) and 0 <= frame_index < len(all_frames):
                        ...
                        try:
                            print(f"\t...image_keypoints: {img_path}::{frame_index}", flush=True)
