...
class GaitSequenceAnalyzer:
    ...
    def analyze_sequence(
        ...
        try:
            ...
            if self.verbose:
                print(f"Tinetti Score: {gait_score}/12")
                print(f"Fall Risk: {risk_level} ({risk_score:.2f})")

            return results

        except Exception as e:
            if self.verbose:
                print(f"Error analyzing sequence {seq_id}: {str(e)}")
            raise
    ...
    def analyze_multiple_sequences(
        ...
        for seq_id, pose_data in sequences_data.items():
            ...
            try:
                ...
                if self.verbose:
                    print(f"Failed to analyze sequence {seq_id}: {str(e)}")
...
def example_usage():
    ...
    print(f"Frames analyzed: {results['pose_data_frames']}")
    print(f"Tinetti Gait Score: {results['tinetti_gait_score']}/12")
    print(f"Fall Risk Level: {results['fall_risk_level']}")
    print(f"Fall Risk Score: {results['fall_risk_score']:.2f}")

    print("\n=== Tinetti Component Scores ===")
