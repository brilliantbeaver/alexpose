...
os.environ["TF_CPP_VMODULE"] = (
    "inference_feedback_manager=0"  # Suppress feedback manager
)
...
# Additional C++ log suppression using Python's logging module
# This catches logs that bypass stderr redirection
# Create a filter to suppress specific MediaPipe/TensorFlow C++ messages
class CPPLogFilter(logging.Filter):
    """Filter to suppress harmless C++ logs from MediaPipe and TensorFlow."""
...
# Apply the filter to root logger and stderr handler
_cpp_filter = CPPLogFilter()
logging.root.addFilter(_cpp_filter)

# Also filter stderr directly if possible
