...
NOTEBOOK_DIR = (
    PROJECT_DIR / "notebooks" / "reflection_equivariance"
)
...
def main() -> None:
    ...
    for name in ("06_cpu_replication.ipynb", "07_gpu_replication.ipynb"):
        ...
        assert '\\"reflection_equivariance\\" / name' in source
    ...
    for name in (
        "build_gavd_notebooks.py",
        "build_replication_notebooks.py",
        "build_amass_notebook.py",
    ):
        path = generator_dir / name
        source = path.read_text(encoding="utf-8")
        compile(source, str(path), "exec")
        assert '"notebooks" / "reflection_equivariance"' in source
        if name == "build_amass_notebook.py":
            assert '"08_amass_training.ipynb"' in source
        else:
