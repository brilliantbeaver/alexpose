...
from gavd6_sjepa.shared_infrastructure.artifact_io import atomic_write_dataframe_csv, sha256_file
...
from .decisions import decide_gate
from .metrics import (
    score_arrays,
...
from .nested_training import verify_fold
from .residual_models import TrainingScaler
from .validity_audits import ValidityAuditRejected, require_audits
        ...
        for seed in config.seeds:
            ...
            for fold in range(5):
                ...
                if getattr(config, "target_version", None) == "training-target-v1":
                    from .joint_models import TargetStandardizer
...
def build_report(root):
    ...
    if protocol_name(run) == "direct-v3":
        from .joint_report import build_joint_report
        return build_joint_report(root)
    if protocol_name(run) == DIRECT_PROTOCOL:
        from .direct_report import build_direct_report
