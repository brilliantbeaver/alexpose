...
from gavd6_sjepa.shared_infrastructure.artifact_io import atomic_write_json, sha256_file
...
def code_fingerprint():
    """Hash the current study layout, required runtime sources and dependency lock."""
    ...
    required_paths = (
        'src/gavd6_sjepa/data_foundations/pose_extraction.py',
        'src/gavd6_sjepa/data_foundations/video_download.py',
        'src/gavd6_sjepa/shared_infrastructure/artifact_io.py',
        'src/gavd6_sjepa/cli.py',
        'pyproject.toml',
        'uv.lock',
        'slurm/future-prediction/audit-targets.sbatch',
        'slurm/future-prediction/build-cohort.sbatch',
        'slurm/future-prediction/build-report.sbatch',
        'slurm/future-prediction/cache-teacher.sbatch',
        'slurm/future-prediction/common.sh',
        'slurm/future-prediction/extract-poses.sbatch',
        'slurm/future-prediction/fit-models.sbatch',
        'slurm/future-prediction/initialize-repair.sbatch',
        'slurm/future-prediction/notebooks/cohort.sbatch',
        'slurm/future-prediction/notebooks/fit.sbatch',
        'slurm/future-prediction/notebooks/report.sbatch',
        'slurm/future-prediction/notebooks/reuse-cache.sbatch',
        'slurm/future-prediction/notebooks/setup.sbatch',
        'slurm/future-prediction/notebooks/teacher-cache.sbatch',
        'slurm/future-prediction/setup.sh',
        'slurm/future-prediction/submit-notebooks.sh',
        'slurm/future-prediction/submit-pipeline.sh',
        'slurm/future-prediction/submit-repair.sh',
    )
    paths = {Path(__file__).parent / name for name in required_modules}
    paths.update(checkout / name for name in required_paths)
    paths.update(Path(__file__).parent.glob("*.py"))
    launchers = checkout / "slurm/future-prediction"
    paths.update(launchers.rglob("*.sh"))
    paths.update(launchers.rglob("*.sbatch"))
    return stable_key("code-layout-v2", *[
        f"{p.relative_to(checkout).as_posix()}:{sha256_file(p)}" for p in sorted(paths)
    ])
...
def load_model_contract(root):
    if read_json(Path(root) / "config/run-contract.json").get("protocol") == REPAIR_PROTOCOL:
        from .joint_models import JointModelContract
