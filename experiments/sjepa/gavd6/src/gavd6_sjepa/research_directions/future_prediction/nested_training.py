...
from gavd6_sjepa.shared_infrastructure.artifact_io import (
    atomic_save_joblib,
    atomic_save_torch,
    sha256_file,
)

from .cohort import assign_source_folds
...
from .controls import controlled_history
from .feature_cache import load_cache
from .residual_models import SkeletonResidualHead, fit_baseline, predict_head, train_head
from .validity_audits import require_audits
...
def fit_outer_fold(root, outer_fold, device="cpu"):
    ...
    if read_json(root / "config/run-contract.json").get("protocol") == "direct-v3":
        from .joint_training import fit_joint_outer_fold
...
def verify_fold(root, fold):
    root = Path(root)
    if read_json(root / "config/run-contract.json").get("protocol") == "direct-v3":
        from .joint_training import verify_joint_fold
