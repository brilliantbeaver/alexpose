...
from ..future_prediction.cache_reuse import read_parent, checked_file
from ..future_prediction.cohort import build_candidates
from ..future_prediction.feature_cache import load_window
from ..future_prediction.nuisance_features import context_nuisance
from ..future_prediction.token_regions import pool_context, pool_target, region_masks, union_box
from ..future_prediction.video_pose import (
    CropGeometry,
    decode_exact_window,
    extract_history,
    alignment_sheet,
)
from ..future_prediction.validity_audits import future_pixel_leakage_test
from ..future_prediction.readiness import verify_readiness_tables
from .cohort import read_study, PROTOCOL
from .readiness import development_media, require_development_media, original_inputs
from .availability import verify_available, require_included_windows, check_selected_file
...
def cache(root, parent=None, device='cuda', adapter=None):
    ...
    for position, row in enumerate(cohort.to_dict('records'), 1):
        ...
        if receipt.exists():
            ...
            if adapter is None:
                from ..future_prediction.vjepa import FrozenVJEPAAdapter
