...
from ..future_prediction.cache_reuse import read_parent, snapshot, verify_snapshot, checked_file
from ..future_prediction.contracts import (
    read_json,
    write_json,
    write_once_json,
    sha256_file,
...
from ..future_prediction.cohort import assign_source_folds
from ..future_prediction.joint_models import JointModelContract
...
def fingerprint():
    """Bind the renamed study, launchers and reservation inputs to their real bytes."""
    directory = Path(__file__).parent
    checkout = Path(__file__).resolve().parents[4]
    from ..future_prediction.contracts import code_fingerprint
    ...
    required_paths = (
        'scripts/research_directions/source_scaling/build_notebook.py',
        'scripts/research_directions/source_scaling/calibrate.py',
        'scripts/research_directions/source_scaling/initialize.py',
        'scripts/research_directions/source_scaling/inspect_notebooks.py',
        'scripts/research_directions/source_scaling/known-exposure.csv',
        'scripts/research_directions/source_scaling/known-exposure.json',
        'scripts/research_directions/source_scaling/run.py',
        'scripts/research_directions/source_scaling/run_stage.py',
        'slurm/source-scaling/build-report.sbatch',
        'slurm/source-scaling/cache-features.sbatch',
        'slurm/source-scaling/common.sh',
        'slurm/source-scaling/fit-models.sbatch',
        'slurm/source-scaling/initialize.sbatch',
        'slurm/source-scaling/inspect-notebooks.sbatch',
        'slurm/source-scaling/prepare-data.sbatch',
        'slurm/source-scaling/submit-stages.sh',
        'slurm/source-scaling/submit.sh',
    )
    paths = {directory / name for name in required_modules}
    paths.update(checkout / name for name in required_paths)
    paths.update(directory.glob("*.py"))
    for folder, suffixes in (
        (checkout / "slurm/source-scaling", ("*.sh", "*.sbatch")),
        (checkout / "scripts/research_directions/source_scaling", ("*.py", "*.csv", "*.json")),
    ):
        for suffix in suffixes:
            paths.update(folder.rglob(suffix))
    return {
        "code_layout": "study-v2",
        "historical_future_innovation": code_fingerprint(),
        **{p.relative_to(checkout).as_posix(): sha256_file(p) for p in sorted(paths)},
    }
...
def freeze(root, parent, sequence_manifest, video_manifest, inspected_manifests,
    ...
    if video_root is not None:
        from .availability import freeze_available
...
def read_study(root, parent_override=None, *, require_software=False):
    ...
    if 'availability-contract.json' in study['config_sha256'] or study.get('cohort_policy') is not None:
        from .availability import COHORT_POLICY, verify_available
