...
def review_alignment(root, reviewer, window_ids, note):
    root = Path(root)
    cohort = load_cohort(root, verify_artifacts=True)
    selected = cohort[cohort.window_id.isin(window_ids)]
    if set(selected.window_id) != set(window_ids) or set(selected.outer_fold) != set(
        range(5)
    ):
        raise ValueError("Record inspected window IDs covering all five folds")
    if not reviewer.strip() or not note.strip():
        raise ValueError("Reviewer and actual inspection notes are required")
    write_once_json(
...
def validate_alignment_review(root, cohort):
    root = Path(root)
    review = read_json(root / "qc/alignment-review.json")
    inspected = cohort[cohort.window_id.isin(review["window_ids"])]
    if (
        review.get("passed") is not True
        or not review.get("reviewer", "").strip()
        or review["manifest_sha256"] != sha256_file(root / "manifests/gate-windows.csv")
        or set(inspected.outer_fold) != set(range(5))
    ):
        raise ValueError(
            "Alignment inspection must cover all five folds and the frozen cohort"
        )
