"""future feature prediction / gate / test training."""
...
from gavd6_sjepa.research_directions.future_prediction.cache_reuse import (
    initialize_cached_run,
    load_reused_cache,
    read_parent,
    snapshot,
    verify_snapshot,
...
from gavd6_sjepa.research_directions.future_prediction.joint_models import (
    JointModelContract,
    JointRidge,
    SupportedInput,
    TargetStandardizer,
...
from gavd6_sjepa.research_directions.future_prediction.joint_report import (
    decide_joint_gate,
    verify_numerics,
)
from gavd6_sjepa.research_directions.future_prediction.joint_training import (
    choose_candidate,
    nested_partition,
    predict_selected,
    verify_joint_fold,
...
from gavd6_sjepa.research_directions.future_prediction.reporting import (
    assemble_oof,
    build_report,
    score_gate,
)
from gavd6_sjepa.research_directions.future_prediction.residual_models import (
    TrainingScaler,
    fit_baseline,
    train_head,
)
...
class CachedRepairIntegrationTests(unittest.TestCase):
    ...
    def test_missing_inherited_cache_or_audit_never_loads_teacher(self):
        import sys
        from gavd6_sjepa.research_directions.future_prediction import cli as fi_entrypoint
        ...
        for filename,handler in (('config/cache-contract.json',fi_entrypoint.cache_main),
            ...
            try:
                with patch.object(sys,'argv',['fi','--run-root',str(root),'--device','cpu']), patch(
                    'gavd6_sjepa.research_directions.future_prediction.vjepa.FrozenVJEPAAdapter.from_run') as teacher:
    ...
    def test_initialization_recovers_without_changing_frozen_parent(self):
        ...
        from gavd6_sjepa.research_directions.future_prediction import cache_reuse as reuse
