"""future feature prediction / scaling / test notebooks."""
...
spec = importlib.util.spec_from_file_location('source_curve_notebook_execution', LAUNCH / 'inspect_notebooks.py')
...
class SourceNotebookTests(unittest.TestCase):
    def test_external_inspection_preserves_entire_frozen_run_and_binds_its_contract(self):
        from gavd6_sjepa.shared_infrastructure.result_catalog import snapshot
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)/'run'; fixture(root)
            external = Path(tmp)/'inspections'
            before = snapshot(root)
            output = runner.execute_notebooks(root, inspection_root=external)
            self.assertEqual(snapshot(root), before)
            self.assertTrue((external/'logs/notebooks'/output.name/'23-execution.json').exists())
            other = Path(tmp)/'other'; fixture(other)
            with self.assertRaisesRegex(ValueError, 'different run'):
                runner.inspection_layout(other.resolve(), external)
            for invalid in (root, root/'inspection', root.parent):
                with self.assertRaisesRegex(ValueError, 'separate'):
                    runner.inspection_layout(root.resolve(), invalid)
    ...
    def test_notebook_only_submission_needs_no_parent_models_or_media(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result=subprocess.run(['bash',str((ROOT/'slurm/source-scaling/submit.sh')),'notebooks','--dry-run'],
                                  env=environment,capture_output=True,text=True)
            self.assertEqual(result.returncode,0,result.stderr)
            commands=[line for line in result.stdout.splitlines() if line.startswith('sbatch ')]
            self.assertEqual(len(commands),1)
            self.assertIn('inspect-notebooks.sbatch',commands[0])
...
''')
            ...
            for exit_code in (0,7):
                result=subprocess.run(['bash',str((ROOT/'slurm/source-scaling/inspect-notebooks.sbatch'))],
                ...
                self.assertEqual(json.loads((directory/'args.json').read_text()),
                                 ['scripts/research_directions/source_scaling/inspect_notebooks.py','--run-root',str(root)])
