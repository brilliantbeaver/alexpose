"""future feature prediction / scaling / test launch."""
...
LAUNCH = ROOT / 'scripts/research_directions/source_scaling'
...
def fixture(directory):
    ...
    return dict(GAVD6_ROOT=str(ROOT), FI_RUN_ROOT=str(directory / 'new study'),
                FI_INSPECTION_ROOT=str(directory / 'inspections'),
...
class LaunchTests(unittest.TestCase):
    ...
    def test_dry_run_accepts_new_root_and_exports_old_jobs_alias(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for mode, count in [('all',6),('prepare',2)]:
                result = subprocess.run(['bash',str((ROOT/'slurm/source-scaling/submit.sh')),mode,'--dry-run'],
                                        env={**os.environ,**env},capture_output=True,text=True)
                self.assertEqual(result.returncode,0,result.stderr)
                commands = [line for line in result.stdout.splitlines() if line.startswith('sbatch ')]
                self.assertEqual(len(commands),count)
                self.assertIn('initialize.sbatch',commands[0])
    ...
    def test_check_is_read_only_and_only_an_empty_development_pool_blocks(self):
        with tempfile.TemporaryDirectory() as tmp:
            env = fixture(Path(tmp))
            command = ['bash', str((ROOT/'slurm/source-scaling/submit.sh')), 'check']
    ...
    def test_missing_recording_is_reported_and_does_not_block_submission_preview(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result=subprocess.run(['bash',str((ROOT/'slurm/source-scaling/submit.sh')),'all','--dry-run'],
...
''')
            ...
            result = subprocess.run(['bash',str((ROOT/'slurm/source-scaling/submit.sh')),'all'],
                                    env={**os.environ,**env},capture_output=True,text=True)
            self.assertEqual(result.returncode,0,result.stderr)
            ...
            self.assertEqual(len((Path(env['FI_RUN_ROOT'])/'logs/submissions.tsv').read_text().splitlines()),5)
            notebook_submissions = Path(env['FI_INSPECTION_ROOT'])/'logs/slurm/submissions.tsv'
            self.assertEqual(len(notebook_submissions.read_text().splitlines()),1)
...
class RetainedStudyIntegrationTests(unittest.TestCase):
    def test_operational_repairs_preserve_frozen_numerical_implementation(self):
        # Compare against the immutable pre-refactor source, so this check
        # remains valid after the renamed files are committed.
        prefix='src/gavd6_sjepa/research_directions/future_innovation_scaling/'
        for old_name, current_name, names in (
                ('fi_scaling_cohort.py', 'cohort.py', {'source_groups','reserve_sources','make_plan'}),
                ('fi_scaling_training.py', 'learning_curve.py', {'compute_curve','verify_fit','prediction_payload'})):
            relative = prefix + old_name
            current = ROOT / 'src/gavd6_sjepa/research_directions/source_scaling' / current_name
            with tarfile.open(ROOT / 'scripts/archive/code_layout_20260913/source.tar.gz') as archive:
                saved = archive.extractfile(relative).read()
            def functions(source):
                return {node.name:ast.dump(node,include_attributes=False) for node in ast.parse(source).body
                        if isinstance(node,ast.FunctionDef) and node.name in names}
            self.assertEqual(set(functions(saved)),names)
            self.assertEqual(functions(saved),functions(current.read_text()),str(current))
    ...
    def test_real_parent_freeze_publish_recovery_and_exact_reservation(self):
        ...
        from gavd6_sjepa.research_directions.future_prediction.cache_reuse import snapshot,verify_snapshot
