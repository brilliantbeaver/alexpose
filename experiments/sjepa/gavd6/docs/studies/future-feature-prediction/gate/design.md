...
For new runs, use the [direct-v2 50-clip gate specification](../../future-innovation/direct-gate-protocol.md).
...
[study overview and five executable tutorials](../).
Use the [HAIC guide](../../../../slurm/future-prediction/README.md) for real execution
...
Before training a skeleton model to learn from a large video model, we need to know whether the skeleton contains a useful part of what we want it to learn. This guide turns the [48-hour gate in Proposal 2](../../../../notes/archive/research-planning/proposals-03/02-future-innovation-distillation.md#the-48-hour-decision-gate) into that first test:
...
These components are now implemented under `research_directions/future_innovation/`, with lazy `gavd6 future-innovation` commands and jobs under `slurm/future-prediction/`. The sketches below describe the scientific contract; use the linked run guide and command-specific `--help` for the implemented interfaces.
...
```text
src/gavd6_sjepa/research_directions/future_prediction/
├── cohort.py
├── video_and_pose.py
├── vjepa_adapter.py
├── token_regions.py
