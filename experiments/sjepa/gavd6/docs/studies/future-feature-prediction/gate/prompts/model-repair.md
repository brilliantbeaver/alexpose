...
The [initial root-cause note](../model-diagnostics.md) is historical. Its early emphasis on zero initialization was superseded by the saved-model evaluation. The older experiment guide and distillation proposal also contain teacher-selectivity requirements that direct-v2 explicitly removed. Identify such differences rather than combining incompatible protocols.
...
- A new protocol document under `docs/studies/future-feature-prediction/` describing the chosen model, preprocessing, controls, selection, seed policy, thresholds, and compatibility decisions before the revised real-data run.
...
- A repair/validation report under `docs/studies/future-feature-prediction/` connecting each confirmed cause to its implementation change and verification evidence, with commands for reproduction and resumption.
...
All `fi_*.py` files below are under `src/gavd6_sjepa/research_directions/future_prediction/`.

| Area | Starting points |
| --- | --- |
| Input scaling and model types | `residual_models.py`; introduce a focused joint-model module if useful. |
| Nested selection and model reload | `nested_training.py` |
| Protocol, configuration, and cache/QC lineage | `contracts.py`, `feature_cache.py`, `readiness.py`, `validity_audits.py` |
| Controls and temporal features | `controls.py` and the revised feature constructor |
| Metrics, verification, reporting, and decisions | `metrics.py`, `reporting.py`, `direct_report.py`, `decisions.py` |
| Notebook execution and inspection | `notebook_workflow.py`, `inspection.py`, and the canonical generator linked above |
| CLI and scheduled execution | `cli.py`, `slurm/future-prediction/` |
