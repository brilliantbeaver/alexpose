...
The latest [available-cohort revision](cohort-validation.md)
...
[short guide](../../../../slurm/source-scaling/README.md) to submit
...
|---|---|
| Audit/reservation | `cohort.py`; explicit exposure registry, participant-connected groups, reserved confirmation role and deterministic fold/subset identities |
| Expanded input stages | `data.py`; existing candidate builder, decoder, pose processing, token pooling, nuisance construction and readiness arithmetic |
| Fitting | `future_innovation_scaling/nested_selection.py` adds normalized penalties and source-group orchestration while importing the original model, controls and selection rules |
| Saved models and reports | `learning_curve.py`; unique fit identities, all candidate records, selected coefficients, preprocessing/donors, predictions, paired uncertainty and an SVG curve |
| CLI | `run_source_learning_curve.py` and the isolated `future_innovation_scaling.cli` module; old gate initialization still requires 50 clips |
| Slurm | Jobs 20–23 under `slurm/source-scaling/` and `submit-source-learning-curve.sh`; separate CPU/GPU stages, five-fold fit array, dependencies and explicit roots |
| Notebook | [23_source_learning_curves.ipynb](../../../../notebooks/source_scaling/23_source_learning_curves.ipynb), generated from `build_source_learning_curve_notebook.py`; inspection does not fit or encode |
...
`slurm/source-scaling/`, with a dedicated script entry point. The
...
.venv/bin/python scripts/research_directions/source_scaling/calibrate.py \
  --output-root work/artifacts/source-learning-curve-20260911/calibration-final-v3
.venv/bin/python scripts/research_directions/source_scaling/build_notebook.py
git diff --check
```

The [execution guide](../../../../slurm/source-scaling/README.md)
provides the automatic HAIC submission path; the
[advanced guide](../../../../slurm/source-scaling/details.md)
...
The [short execution guide](../../../../slurm/source-scaling/README.md)
now uses `FI_RUN_ROOT` and the existing README data/model variables. The new
`slurm/source-scaling/submit.sh all` schedules a CPU initialization
...
been removed from `nested_selection.py`. The older calibration correctly failed
...
relocation requirements remain in the [advanced guide](../../../../slurm/source-scaling/details.md).
...
uv run --no-sync python scripts/research_directions/source_scaling/inspect_notebooks.py \
...
[`notebooks/future_innovation/`](../../../../notebooks/source_scaling/23_source_learning_curves.ipynb),
alongside the Experiment 0 notebooks. The separate laterality-to-distillation
tutorials 19–22 live in `notebooks/target_accessibility/`. Their numbers and
