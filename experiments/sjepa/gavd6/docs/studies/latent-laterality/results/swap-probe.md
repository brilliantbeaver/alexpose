...
![The E0 experimental flow from clean windows to five correction arms and a frozen common readout](../development/images/swap-probe-01-experimental-flow.svg)
...
| File | Purpose |
| --- | --- |
| `src/gavd6_sjepa/research_directions/reflection_equivariance/swap_probe.py` | Typed corruptions, invariant boundary features, structured path inference, frozen encoding, metrics, and artifact writing |
| `uv run gavd6 swap-probe run` | Local/HAIC command-line entry point |
| `slurm/reflection-equivariance/evaluate-swaps.sbatch` | Four-hour H100 validation job |
| `tests/reflection_equivariance/test_bilateral_correction.py` | Algebra, corruption, parity, inference, leakage, and end-to-end smoke tests |
...
sbatch --account="$HAIC_ACCOUNT" --export=ALL \
  slurm/reflection-equivariance/evaluate-swaps.sbatch
```
...
![E0 separates consequence, leakage, simple correction, and uncertainty-value decisions](../development/images/swap-probe-02-decision-gates.svg)
