...
> [completed findings](../results/validation.md) for
> the current decision and the [HAIC execution record](../execution/haic.md)
...
| [swap-probe.md](../results/swap-probe.md) | The E0 code and verification guide; the local implementation suite is green |

---

## 1. The program at a glance

![The seven experiments and the gate each one guards](../development/images/experiments-01-program-map.svg)
...
| `src/gavd6_sjepa/research_directions/reflection_equivariance/jepa.py` | Available | Standard and paired fixed-reflection model components |
| `src/gavd6_sjepa/research_directions/reflection_equivariance/amass_training.py` | Available | Streaming dataset, losses, checkpointing, evaluation |
| `uv run gavd6 amass train` | Available | AMASS training entry point |
| `src/gavd6_sjepa/research_directions/reflection_equivariance/swap_probe.py` | Available | E0: typed corruption, path inference, frozen encoding, metrics |
| `uv run gavd6 swap-probe run` | Available | E0 command-line entry point |
| `slurm/reflection-equivariance/evaluate-swaps.sbatch` | Available | E0 four-hour H100 job |
| `slurm/reflection-equivariance/train-amass.sbatch` | Available, not matrix-safe | Selects seed 7 and standard S-JEPA only; it does **not** launch all arms |
...
![How data flows once the setup is complete](../development/images/experiments-02-data-pipeline.svg)
...
artifact set in [swap-probe.md](../results/swap-probe.md#7-output-artifacts).
...
| **Full guide** | [swap-probe.md](../results/swap-probe.md) |
...
![Arms ordered by how much each one is told](../development/images/experiments-03-arm-ladder.svg)
...
[swap-probe.md](../results/swap-probe.md). The short
...
sbatch --account="$HAIC_ACCOUNT" --export=ALL slurm/reflection-equivariance/evaluate-swaps.sbatch
```
...
![From overlapping windows to an identity-clustered confidence interval](../development/images/experiments-04-aggregation.svg)
