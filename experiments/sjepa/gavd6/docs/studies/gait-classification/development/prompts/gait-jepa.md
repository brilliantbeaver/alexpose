...
Carefully and thoughtfully read through the attached documents to understand the research directions tried so far. Next, you are to synthesize the methods, experiments, and results from notebooks 00 to 06 in `notebooks/gait_classification`. Ultrathink on how to creatively extend the experiments toward an insightful new research direction. You must be grounded in hard evidence, logical reasoning, compelling inferences, and insightful analysis of real data.
...
**Output**: You should store all of your proposals in `notes/archive/research-planning/proposals-02`.
