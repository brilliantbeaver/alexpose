...
## Run a small comparison before adding complexity

Start with the existing window length, patch size, 50% nominal mask budget and a reviewed body12 dataset to isolate the policy change. Match the **realized** hidden fraction among available tokens, not only the configuration value. The six-arm screen crosses two pretraining objectives with three masking policies:
...
This screen compares practical policies; it cannot by itself attribute a gain to anatomical connectivity. If graph masking helps, add a **topology-shuffled control** that matches group sizes, temporal intervals, mask counts and marginal joint exposure while breaking anatomical adjacency. Compare against random-joint intervals with the same run-length distribution. Reject or explicitly report residual mismatches when the constraints cannot be matched. A fixed anatomical mask is an optional historical failure demonstration, not a necessary expensive training arm and not a fair connectivity control because its marginal exposure is different.

Keep the direct end-to-end model, calibration and temporal filtering on the same population. If adding the selected masking augmentation to direct training, give it a new arm name and match the observation distribution and supervision. Retain initialized and shuffled-reference feature controls for the selected JEPA policy before claiming a benefit from aligned representation learning. Separate the masking experiment from the paired-change-loss experiment in the [main proposal](../README.md), then test an interaction only if both warrant further study.

During the screen, hold people, clips, reference mapping, update budgets, readout capacity, tuning budget and evaluation conditions fixed. Report compute as well as updates. Use one seed only to check execution and identify obvious failures, then repeat the prespecified finalists with the same declared seed set; a one-seed screen supplies no statistical confirmation. A small CPU mask/gradient check should precede GPU jobs.
...
Proceed to a larger experiment only after the graph adapter, coverage bank, preprocessing information check and gradient smoke test pass. If gains disappear after matching temporal persistence or direct supervision, report that boundary and retain the simpler policy. No result from the current audit establishes that graph masking will improve JEPA or achieve statistical significance.
