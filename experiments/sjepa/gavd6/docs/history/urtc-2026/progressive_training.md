...
> **Historical record.** This document describes the pre-repair five-stage
> GAVD run preserved in this package. Its checkpoints and artifact contract are
> not part of the current workflow; begin at the [current study
> index](../../studies/) for active instructions.
...
The Stage 4 state is written into a directory keyed by its verified model-state SHA-256,
together with an explicit `training_manifest.json`; downstream notebooks select that manifest
rather than a fixed final-checkpoint filename. Total training for a given run is recorded in
that manifest and its adjacent training history.
