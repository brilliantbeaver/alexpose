...
> **Historical tutorial.** This account ends with the pre-repair five-stage
> GAVD run. The referenced `cache/artifacts/real` artifact contract is not in
> this checkout; use the [current study index](../../studies/) for active code
> and protocols.
...
- the original `cache/artifacts/real/classifier_contract.json` (not included in
  this checkout);
...
If the original historical `cache/artifacts/real` contract has been restored,
rebuild all publication figures from the repository root:

~~~bash
cd docs/history/urtc-2026
MPLCONFIGDIR=../../../cache/matplotlib ../../../.venv/bin/python make_figures.py
MPLCONFIGDIR=../../../cache/matplotlib ../../../.venv/bin/python make_evolution_figures.py
~~~
