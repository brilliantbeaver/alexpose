...
`outputs/archive/run1/seed-7_standard.pt` is an immutable historical training artifact. It was produced by commit `c2385c125d41698451486cc7cc30f1892d3ec773`, not by the current packaged implementation. The current package has no `standard` variant, so it must not be used to load this checkpoint.
...
It writes a new sidecar at `outputs/archive/run1/pilot_probe_c2385c1/compatibility_report.json`; it never writes the `.pt` checkpoint. Re-running does not overwrite that report unless `--overwrite-report` is supplied explicitly.
