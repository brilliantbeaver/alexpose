...
Final checkpoints are selected through notebook 04's explicit `training_manifest.json`.
The manifest records the tensor-state SHA-256, cohort, extraction run, and training run;
reports must not use a fixed checkpoint filename or a data/config fingerprint as a weight ID.
