...
A source configuration uses `mode: "source"` and an audited `bundle` path, plus explicit model, arms, seeds and compute scope. Follow the [HAIC guide](../../slurm/synthetic-training-v2/README.md) to check the environment, prepare paired data, and submit source stages through Slurm. `execute_notebooks.py` supports CPU fixtures only; the source launcher executes Python stages and saves reports and artifacts. Source preparation is a prerequisite: the notebooks do not obtain licenses, render motions, run pose extractors, certify anatomy or create independent temporal annotations. Large source fitting requires those reviews and the measured all-stage GPU scope; the default authorized GPU budget is zero. No notebook submits jobs.
