...
Noise removal measures squared 3D error reduction at **observed joints only**. Missing coordinates are interpolated for model input, so their completion error is reported separately as `completion_mse_m2`. The summary also includes observed-joint error, all-joint error and the missing fraction. Both methods must reach the repair target on event-plus-noise cases as well as in the overall noisy-case average.
...
Read renderer-reference flow error with its coverage columns in notebook 02. Visibility is conservative: some surface points near unresolved triangle boundaries are excluded. `flow_reference_coverage_foreground` gives the fraction of visible source-body pixels with valid reference transport; `flow_reference_coverage_all_pixels` uses every source-image pixel as the denominator. Low error over a small fraction of the body is limited evidence.
...
The implementation review changed the repair score from all-joint error to observed-joint error. Existing fitted gates can be recalibrated with `workflow.calibrate(cfg)` and reevaluated if final evaluation remains unopened. Old calibration files are rejected because their strengths used the previous score. Start a fresh run for the corrected renderer, and treat any previously inspected final set as development evidence.
