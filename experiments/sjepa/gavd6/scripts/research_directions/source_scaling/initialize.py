...
from gavd6_sjepa.research_directions.future_prediction.cache_reuse import read_parent
from gavd6_sjepa.research_directions.source_scaling.cohort import (
    freeze, read_study, fingerprint, reserve_sources)
from gavd6_sjepa.research_directions.source_scaling.data import seal, verify_seal
...
def settings(env):
    ...
    return dict(checkout=checkout, root=root, parent=parent, full=full,
                launcher=checkout / 'scripts/research_directions/source_scaling')
...
def calibrate(cfg, runner=subprocess.run):
    ...
    runner([sys.executable, str(cfg['checkout'] / 'scripts/research_directions/source_scaling/calibrate.py'),
    ...
    try:
        ...
        if args.command in ('preflight', 'check'):
            ...
            if not (cfg['root'] / 'config/study.json').exists():
                ...
                from gavd6_sjepa.research_directions.source_scaling.cli import status
                print(json.dumps(status(cfg['root'], cfg['parent']), indent=2))
        else:
            from gavd6_sjepa.research_directions.source_scaling.learning_curve import report
