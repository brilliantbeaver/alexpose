...
if MODE == "execute":
    from gavd6_sjepa.research_directions.future_prediction.notebook_workflow import (
...
def opening(number, title, text):
    return [
        ...
        **Run this notebook independently in a fresh kernel.** The default
        ...
        Use the [notebook HAIC launchers](../../slurm/future-prediction/notebooks/README.md)
        ...
        [Study overview](../../docs/studies/future-feature-prediction/gate/history.md) ·
...
def ending(number, text):
    ...
    link = (f"Continue with [{NAMES[f'{following:02d}']}]({NAMES[f'{following:02d}']})."
            if following < 5 else
            "Return to the [study overview](../../docs/studies/future-feature-prediction/gate/history.md) to record the next decision.")
...
if MODE == "teach":
    ...
    from gavd6_sjepa.research_directions.future_prediction.metrics import score_arrays
...
'''), code('''
if MODE != "teach":
    from gavd6_sjepa.research_directions.future_prediction.contracts import read_json
...
'''), code('''
from gavd6_sjepa.research_directions.future_prediction.contracts import FRAME
...
if MODE != "teach":
    ...
    if run_path.is_file() and read_json(run_path).get('protocol')=='direct-v3':
        from gavd6_sjepa.research_directions.future_prediction.cache_reuse import load_reused_cache
...
'''), code('''
from gavd6_sjepa.research_directions.future_prediction.contracts import FRAME
from gavd6_sjepa.research_directions.future_prediction.token_regions import context_indices
...
'''), code('''
if MODE=='teach':
    from gavd6_sjepa.research_directions.future_prediction.joint_calibration import exact_calibration
    from gavd6_sjepa.research_directions.future_prediction.joint_models import JointRidge
...
'''), code('''
if MODE!='teach':
    from gavd6_sjepa.research_directions.future_prediction.inspection import prediction_fit_tables
    from gavd6_sjepa.research_directions.future_prediction.contracts import read_json
...
'''), code('''
if MODE=='teach':
    from gavd6_sjepa.research_directions.future_prediction.joint_report import decide_joint_gate
