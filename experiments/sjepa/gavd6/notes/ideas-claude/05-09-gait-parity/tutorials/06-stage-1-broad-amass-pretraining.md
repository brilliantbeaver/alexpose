...
| `gait_parity_jepa.py` | Yes | The existing 33-landmark GAVD feasibility experiment only |
...
Do not feed raw AMASS or converted core-11 arrays to `gait_parity_jepa.py`. That module assumes 33 MediaPipe landmarks, MediaPipe bilateral pairs, and a different reflection channel. Padding 11 joints to 33 would execute but would not be the proposed experiment.
