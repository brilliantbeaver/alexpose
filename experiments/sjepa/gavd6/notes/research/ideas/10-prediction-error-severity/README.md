...
Each token turns a 4-frame by 3-coordinate (x, y, relative z) 12-vector into a 64-number embedding through a linear layer. A "12-vector" is a list of 12 numbers (4 frames times 3 coordinates). The embedding dimension is 64 by default; the encoder is a small Transformer of depth 2 with 4 heads.
...
4. Group tokens into three mechanism channels (deterministic, frozen before results).
   ...
   - RHYTHM channel: relative error concentrated in cycle-to-cycle timing structure across the 16 time positions (the temporal layout of surprise), the skeleton proxy for stride-time variability. Note the documented limit that stride-time CV is not linearly decodable from roughly 2-second windows, so the rhythm channel is the weakest and we pre-register it as exploratory, not confirmatory. It is exercised in-cohort by the exploratory timing-jitter injection (step 5) and corroborated at the reach tier by the label-level gaitpdb cross-modal check; the primary verdict does not depend on it.
...
- FINGERPRINT hygiene: bind every number to the new normal-only checkpoint's own fingerprint, kept separate from `d0acc262` and the observed `dba24a` lineage.
