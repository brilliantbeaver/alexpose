...
The most practical sequence is idea 3 first, then ideas 2 and 1. Idea 3 supplies the measurement instrument. Idea 2 supplies controlled causal training data. Idea 1 uses both to make the broad world-model claim. Ideas 4 through 7 can be developed without weakening the core paper.
...
5. Can a useful result, including a rigorous null, be produced by one or two researchers in 16 to 24 weeks?
...
- **Week 4:** frozen manifests, conversion tests, and matched forward and inverse baselines.
- **Week 8:** stochastic inverse generation with valid joint limits in at least 95 percent of samples.
- **Week 12:** at least 10 percent improvement in one primary trajectory endpoint and at least 5 contact-F1 points.
- **Week 18:** at least 20 percentage points more goal success in held-out MuJoCo objects, with at least 90 percent of baseline diversity.
- **Week 24:** real cross-dataset transfer. If this fails, report a simulator-bounded method and do not claim broad human-object generalization.
...
Within 20 weeks, does paired counterfactual pretraining reduce held-out trajectory, contact, momentum, and force error by at least 15 percent relative to an equal-sized unpaired domain-randomization baseline on both out-of-range single-parameter interventions and unseen two-parameter compositions, while retaining equal in-distribution prediction accuracy?
...
- **Week 4:** preregister parameter ranges and separate fixed-action from reoptimized counterfactuals.
- **Week 8:** force balance and numerical integration checks are within 5 percent on at least 95 percent of retained simulations.
- **Week 12:** paired training beats equal-data unpaired training outside the parameter range.
- **Week 16:** the gain survives two-factor compositions and a new embodiment.
- **Week 20:** at least 80 percent correct direction on real interventions. Otherwise, remove the real-world causal claim.
...
- **Week 4:** frozen split manifest, six corruptions, two held-out families, and heuristic baselines.
- **Week 8:** offline projection converges on at least 90 percent of validation clips, with failures logged rather than discarded silently.
- **Week 12:** Spearman correlation at least 0.60 and AUROC at least 0.80 on real estimator failures.
- **Week 18:** expected calibration error at most 0.05 and Brier improvement at least 15 percent on unseen corruptions.
- **Week 24:** external-lab replication and monotonic improvement when the energy reranks generated motion.
...
- **Week 4:** deterministic and conditional-diffusion baselines on identical tokens and splits.
- **Week 8:** top-20 generation improves minADE at least 15 percent without degrading mean ADE more than 10 percent.
- **Week 12:** noncollapsed contact-mode use and better proper score than a flat stochastic decoder.
- **Week 16:** physics reranking cuts residuals at least 30 percent while retaining 90 percent of diversity.
- **Week 20:** more executable modes on held-out objects. Without that result, report a motion generator, not a mechanics-conditioned contact-mode generator.
...
- **Week 4:** a public modality-by-dataset table and exact coordinate, timing, and contact schema.
- **Week 8:** the alphabet reconstructs object effect and contact phase across two training embodiments.
- **Week 12:** few-shot adaptation reaches the same success with at most half the labels of the strongest baseline.
- **Week 18:** at least 15 percentage points more execution success at a fixed small label budget.
- **Week 24:** held-out embodiment and object-category transfer, with camera and identity probes controlled.
...
- **Week 4:** 500-frame audit and frozen raw-metric benchmark.
- **Week 8:** mask IoU at least 0.85, depth AbsRel at most 0.20, scale drift at most 5 percent, and object-centroid MAE at most 15 cm on validation.
- **Week 12:** confidence ECE at most 0.05 and measurable gain from disagreement features.
- **Week 16:** at least 10 percent trajectory gain over the best single-tool pipeline.
- **Week 20:** gain survives a new RGB-D dataset and improves force or contact estimation. Otherwise, keep the method as a calibrated perception front end, not a world-model result.
...
- **Week 4:** frozen tasks, deterministic schemas, cached outputs, cost model, and safety policy.
- **Week 8:** prompting and supervised baselines with full trace audit.
- **Week 12:** sandboxed RL and adversarial tests for loops, null output, fabricated confidence, and cost avoidance.
- **Week 16:** frontier improvement over fixed and supervised routers at the two frozen budgets, with invalid calls below 2 percent; report distance from both endpoint references.
- **Week 20:** blind human-authored tasks with changed wording and tool schemas.
...
### Package A: the rigorous 16 to 24 week paper
...
### Package B: the flagship 9 to 12 month paper
...
### Package C: a high-upside follow-on
...
## Integrated 24-week execution plan

This accelerated plan assumes two researchers, access to accelerator compute, and parallel simulator and data work. A single researcher should execute Package A first and treat Wrench-JEPA as a second project.

![A gated 24-week execution plan](./images/10_roadmap.svg)

| Weeks | Research track | Engineering track | Gate |
|---|---|---|---|
| 1-2 | Freeze claims, splits, corruption families, and counterfactual ranges | Reconcile GAVD fingerprints; bind Core11 artifacts; create tiered schema | Every row has identity, source, units, tier, and hash |
| 3-4 | Establish raw, random, heuristic, and matched model baselines | AddBiomechanics loader; force-plate validation harness; simulator smoke suite | No subject, trial, or dynamics-pass leakage |
| 5-8 | Train and calibrate feasibility energy | Cache OpenSim/Moco fits; generate MuJoCo minimal pairs | Projection converges on at least 90 percent of validation clips |
| 9-12 | Compare paired versus unpaired counterfactual pretraining | Implement typed human, object, contact, wrench tokens | Out-of-range gain and score calibration clear preregistered gates |
| 13-16 | Train matched Wrench-JEPA forward and inverse arms | Build stochastic contact decoder and execution harness | Contact and trajectory gain over strongest matched baseline |
| 17-20 | Held-out object, parameter composition, and embodiment tests | Error analysis; scale and simulator sensitivity | Executable goal success gain survives hard holdouts |
| 21-24 | Cross-dataset real test and sealed analysis | Reproduce from clean environment; finalize artifacts | Claim is reduced or expanded strictly from sealed outcomes |
...
Start with the Force-Calibrated Feasibility Energy. It converts “looks physically plausible” into a testable measurement and can falsify later generative claims. In parallel, build paired MuJoCo counterfactuals and the typed wrench schema. Proceed to full Wrench-JEPA only if both prerequisites pass their week-12 gates.
