...
> **Historical portfolio: 12–13 September 2026.** This page preserves the recommendation at that stage. Start with the [current research agenda](../../README.md) for the later decisions. Shared references and the original motion-preservation diagram remain available to the active study.
...
![Seven alternatives and the recommended order](figures/proposal-comparison.svg)
...
| **2** | [P7. Keep motion beyond keypoints](proposals/motion-beyond-joints.md) | 3 | 5 | 3 | 5 | Preserve visible surface transport omitted by joint positions, then improve forecasts with a few flow tokens. |
...
The literature review also changed the recommendation. Student-attainable targets, criteria for distillation benefit, gait health foundation models, motion personalization, physical scoring and constrained motion generation all have close precedents. Each proposal names those precedents and states the additional result needed to earn a contribution. The [access and literature ledger](../../references/papers-and-checkpoints.md) records the requested inspiration papers, recent collisions and usable checkpoints.

Whole-body AMASS is the main controlled motion resource. GAVD is the real-world video resource and the primary classification dataset whenever classification is used. Neither GAVD binary classification nor inferred clinical forces is a headline. Exact model schemas, participant/source grouping and prefix-only preprocessing are specified in the [evidence and execution contract](../../references/experiment-guidelines.md).

## Reading order and review record

Read P1, P7 and the common execution contract first. Read P2 and P6 if deciding between alternatives. P3 and P5 are the closest methodological continuations of the existing code. P4 is the distributional hypothesis the mean-based study did not test. The earlier [constraint-conflict draft](proposals/conflicting-motion-requests.md) remains archived with its review, outside the final seven.

The portfolio includes **15 original SVG diagrams**, two per proposal plus the ranking map. They are conceptual diagrams, not invented experimental curves. The [adversarial review and revision record](../../references/review-summary-2026-09-13.md) records scientific and visual corrections. The [figure generator](figures/generate_figures.py) and [layout checks](figures/layout-checks.json) make the visual assets reproducible.

Research memos preserve the reasoning behind the selection: [local evidence](reviews/evidence-and-novelty.md), [gait models and data](reviews/gait-models-and-data.md), [world models](reviews/world-models.md), and [portfolio critique](reviews/proposal-comparison.md). These are working evidence records; the seven proposal files and this decision page are the final recommendations.

## Archive guide

The proposal text above is a historical decision record. This guide groups its supporting files so readers can find the reasoning without treating an earlier recommendation as current.

| Review topic | Files |
| --- | --- |
| Evidence and research context | [Evidence and novelty](reviews/evidence-and-novelty.md), [gait models and data](reviews/gait-models-and-data.md), [world models](reviews/world-models.md), [proposal comparison](reviews/proposal-comparison.md) |
| Research methods | [Motion repair and ambiguity](reviews/motion-repair-and-ambiguity.md), [teaching and prediction](reviews/teaching-and-prediction.md), [conflicting motion requests](reviews/conflicting-motion-requests.md) |
| Optical-flow analysis | [Measurements](reviews/optical-flow-measurements.md), [world models](reviews/optical-flow-and-world-models.md), [research options](reviews/optical-flow-research-options.md), [motion beyond joints](reviews/motion-beyond-joints.md) |
| Figure review stages | [Initial](reviews/figures-initial.md), [teaching and prediction](reviews/figures-teaching-and-prediction.md), [conflicting requests](reviews/figures-conflicting-requests.md), [motion beyond joints](reviews/figures-motion-beyond-joints.md), [final](reviews/figures-final.md) |

The [original research brief](research-brief.md) records the request that produced this portfolio. It is historical source material. The [joint-preserving rotation check](checks/verify_joint_preserving_rotation.py) and its [saved output](checks/joint-preserving-rotation-check.json) concern ideal rigid-chain geometry, not an AMASS or model experiment.

The [validation receipt](validation.json) is unchanged historical evidence from September 13. Its filenames and counts describe the layout checked then; it is not a validation of today's directory structure. Review prose also retains quoted draft filenames. Their figure identifiers correspond to these current stems:

| Historical figure identifier | Descriptive filename stem |
| --- | --- |
| `00-portfolio` | `proposal-comparison` |
| `01` (P1) | `motion-preservation` |
| `02` (P2) | `motion-ambiguity` |
| `03` (P3) | `distillation-value` |
| `04` (P4) | `probabilistic-forecasting` |
| `05` (P5) | `response-distillation` |
| `06` (P6) | `cross-activity-prediction` |
| `07` (replacement P7) | `motion-beyond-joints` |
| `07-constraints` (rejected candidate) | `conflicting-requests` |

SVGs are in [figures](figures/); PNGs and contact sheets are in [figures/previews](figures/previews/). Each proposal has `-mechanism` and `-experiment` images. From the repository root, run `.venv/bin/python notes/research-agenda/archive/2026-09-13/figures/generate_figures.py` to regenerate the final seven-proposal set. The two rejected-candidate diagrams remain preserved assets and are not regenerated by that script.
