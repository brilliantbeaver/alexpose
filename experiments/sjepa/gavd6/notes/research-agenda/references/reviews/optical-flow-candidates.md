...
Root's [algebra script](verify_alias_algebra.py) and [receipt](alias_sanity_check.json) are independently inspectable chain checks. They do not exercise SMPL, actual tensor conversion, rendering or estimation. Exact-input tolerance must be close to numerical error and compared with the outcome separation; setting a loose tolerance after seeing results invites a false impossibility claim.
