...
Optical flow strengthens the leading proposal and supplies a better replacement for the weakest candidate. The final seven now include **[P7: Keep motion that the skeleton cannot explain](../proposals/motion-beyond-keypoints.md)**. The old constraint-conflict idea is preserved as a [rejected reserve](reviews/rejected-constraint-conflicts.md).
...
The [rigid-chain algebra check](reviews/verify_alias_algebra.py) verifies the proposed compensated-rotation construction in 100 randomly generated kinematic configurations. Joint positions remain equal to numerical precision while an off-axis material point moves. Its [saved output](reviews/alias_sanity_check.json) is explicitly a mathematical sanity check, not an AMASS, SMPL, optical-flow or forecasting result.

The first 24-hour experiment must establish equality of the actual S-JEPA tensors after the full body-model and preprocessing pipeline. The 48-hour gate must show that estimated flow recovers useful visible evidence and that the natural-motion task survives the strongest simple baselines. The detailed [world-model review](reviews/optical-flow-world-models.md), [measurement review](reviews/optical-flow-measurement.md) and [candidate audit](reviews/optical-flow-candidates.md) record sources and unresolved conditions.
