...
d.save("00-portfolio.svg")

(OUT/"layout-checks.json").write_text(json.dumps(CHECKS,indent=2))
if any(x["overlaps"] for x in CHECKS):
    raise RuntimeError("Text overlap or viewbox overflow. Inspect figures/layout-checks.json.")
...
for kind in ("mechanism","experiment"):
    ...
    for i in range(1,8):
        im=Image.open(PREVIEW/f"{i:02d}-{kind}.png").convert("RGB")
