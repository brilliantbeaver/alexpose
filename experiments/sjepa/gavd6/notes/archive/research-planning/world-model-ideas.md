...
![The proposed shift from pose prediction to causal contact prediction](figures/00_thesis.svg)
...
![Four increasingly strong tests of physics understanding](figures/01_physics_ladder.svg)
...
![Wrench-JEPA uses the same state for forward and inverse prediction](figures/02_wrench_jepa.svg)
...
![Paired counterfactuals isolate one physical cause at a time](figures/03_counterfactual_pairs.svg)
...
![Measured forces calibrate a decomposable physical feasibility energy](figures/04_feasibility_energy.svg)
...
![A discrete contact plan organizes multiple continuous motion futures](figures/05_contact_mode_generator.svg)
...
![A contact and impulse alphabet bridges human and robot embodiments](figures/06_action_alphabet.svg)
...
![Multiple estimators update a posterior that separates camera gauge from physical state](figures/07_gauge_aware_4d.svg)
...
![The agent calls a tool only when expected information value exceeds cost](figures/08_voi_agent.svg)
...
![The seven directions form a staged research stack](figures/09_integrated_stack.svg)
...
![A gated 10-day GPU execution plan](figures/10_roadmap.svg)
