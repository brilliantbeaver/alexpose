...
| --- | --- | --- |
| Command router | `src/gavd6_sjepa/cli.py`, `scripts/navigation_guide.md` | One `gavd6` command exposes AMASS, GAVD, laterality, swap-probe, and notebook-validation jobs. |
...
| Shared contracts | `src/gavd6_sjepa/shared_infrastructure/artifact_io.py` | Provides atomic saves and SHA-256 fingerprints. |
| Notebooks | `notebooks/gait_classification/00` through `06`, `notebooks/amass/`, `notebooks/signed_laterality/`, `notebooks/reflection_equivariance/`, `notebooks/force_prediction/01_strokepig_probe.ipynb` | Foundations reproduce the historical GAVD96 curriculum. Experiment notebooks cover signed laterality and reflection equivariance. Executed copies live under `artifacts/notebook_runs/`. |
...
- `notebooks/reflection_equivariance/09_gavd_frozen_probe.ipynb` exists but is absent from `notebooks/README.md`. Is it generated, hand-maintained, or retired?
