...
> Historical only. This instruction predates the completed 12-target, five-stage augmented-normal run and its corrected evaluation. Its old checkpoint, 10-target, and 61.9% accuracy assumptions must not be reported as current. The paths embedded in the prompt are retained verbatim as a snapshot and are not current repository locations. See the [URTC historical package](../../../docs/history/urtc-2026/README.md) for its revision ledger; use [current study documentation](../../../docs/studies/README.md) for execution decisions.
