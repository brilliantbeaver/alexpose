...
submit_cached() {
  ...
  job="$(sbatch "${options[@]}" "$GAVD6_ROOT/slurm/future-prediction/$script")" || return 1
  job="${job%%;*}"
  [[ "$job" =~ ^[0-9]+$ ]] || { echo "Invalid sbatch response: $job" >&2; return 1; }
  printf '%s\t%s\t%s\n' "$label" "$job" "$dependency" >> "$FI_RUN_ROOT/logs/cached-submissions.tsv"
  printf '%s\n' "$job"
}
init="$(submit_cached init initialize-repair.sbatch '')"
fit="$(submit_cached fit fit-models.sbatch "afterok:$init")"
report="$(submit_cached report build-report.sbatch "afterany:$init:$fit")"
