...
was specified after that rejection. See the [study overview](../../docs/studies/future-feature-prediction)
and [review record](../../docs/studies/future-feature-prediction/gate/execution-history.md).

To execute and retain the five notebooks themselves, use the separate
[notebook HAIC path](notebooks/README.md): `submit-notebooks.sh all` schedules jobs
...
[notebook guide](notebooks/README.md) describes supported paths and the single shared
...
cd "$GAVD6_ROOT"
bash slurm/future-prediction/setup.sh
```
...
```bash
bash slurm/future-prediction/submit-pipeline.sh all
```
...
```bash
bash slurm/future-prediction/submit-pipeline.sh prepare
```
...
```bash
bash slurm/future-prediction/submit-pipeline.sh compute
```
...
attempts require new IDs and the appropriate current protocol.
...
```bash
.venv/bin/python -m gavd6_sjepa.cli future-innovation calibrate-repair \
  --output work/artifacts/future-innovation-repair-2026-09-11/calibration-final.json

.venv/bin/python -m gavd6_sjepa.cli future-innovation init-cached-run \
...
.venv/bin/python -m gavd6_sjepa.cli future-innovation run-gate \
  --run-root outputs/future-innovation-direct-v3-dev-20260911 --device cpu
.venv/bin/python -m gavd6_sjepa.cli future-innovation score-gate \
  --run-root outputs/future-innovation-direct-v3-dev-20260911
.venv/bin/python -m gavd6_sjepa.cli future-innovation build-report \
  --run-root outputs/future-innovation-direct-v3-dev-20260911
.venv/bin/python -m gavd6_sjepa.cli future-innovation verify-repair \
  --run-root outputs/future-innovation-direct-v3-dev-20260911
```
...
cd "$GAVD6_ROOT"
bash slurm/future-prediction/submit-repair.sh
```
...
[README.md](../source-scaling/README.md). It reserves confirmation
recordings before processing and retains the repaired joint model. After setting
its paths, run `bash slurm/source-scaling/submit.sh all`.
