...
`bash slurm/source-scaling/submit.sh notebooks` with
`FI_RUN_ROOT` set to the source study. That executes
[notebook 23](../../../notebooks/source_scaling/23_source_learning_curves.ipynb); `all` in the
same launcher also executes it after the pipeline. See the
[source learning-curve guide](../../source-scaling/README.md). The commands below are
...
jobs `01`–`06` and `submit-pipeline.sh` remain the direct CLI path. Both paths
...
pinned V-JEPA checkout and checkpoint, and run `setup.sh`. This
...
[investigation](../../../docs/studies/future-feature-prediction/gate/execution-history.md#run-haic-gojuxseb-verified-audit-rejection)
...
```bash
uv run --no-sync python scripts/research_directions/future_prediction/execute_notebook.py \
...
See the [tutorial maintenance guide](../../../scripts/research_directions/future_prediction/README.md)
for generation and checks. `verify_notebooks.py --pipeline-smoke`
...
```bash
.venv/bin/python scripts/research_directions/future_prediction/execute_notebook.py \
