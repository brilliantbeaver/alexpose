...
Use the same environment variables as [README.md](../future-prediction/README.md). The launcher below
...
export FI_RUN_ROOT="$GAVD6_ROOT/outputs/studies/future-innovation/source-learning-curve-available-dev-20260912"
cd "$GAVD6_ROOT"
```
...
The launcher also accepts `FI_PARENT_ID=gate-v2` and a new or existing
`FI_RUN_ID`; unset the corresponding explicit root when switching to IDs. New
...
```bash
bash slurm/source-scaling/submit.sh check
```
...
```bash
bash slurm/source-scaling/submit.sh all
```
...
as described in the [advanced instructions](details.md#additional-exposure-or-participant-information)
...
```bash
bash slurm/source-scaling/submit.sh status
```
...
stored inside a run remain preserved there.

`all` executes [notebook 23](../../notebooks/source_scaling/23_source_learning_curves.ipynb)
automatically. To execute it again for an existing
study, without rerunning data processing or fitting:

```bash
bash slurm/source-scaling/submit.sh notebooks
```
...
```bash
bash slurm/source-scaling/submit.sh fit
```
...
[advanced instructions](details.md). The
[available-cohort validation](../../docs/studies/future-feature-prediction/scaling/cohort-validation.md)
