...
mode="${1:-all}"
dry="${2:-}"
case "$mode" in all|prepare|compute|fit|report) ;; *) echo 'Usage: submit-stages.sh {all|prepare|compute|fit|report} [--dry-run]' >&2; exit 2;; esac
...
if [[ "$mode" == all || "$mode" == prepare ]]; then submit_stage prepare-data.sbatch pose; fi
...
if [[ "$mode" != prepare ]]; then submit_stage build-report.sbatch report; fi
