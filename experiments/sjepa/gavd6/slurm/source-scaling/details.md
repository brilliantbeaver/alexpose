...
For ordinary HAIC execution, use the [short guide](README.md).
...
## Additional exposure or participant information

The automatic launcher combines the bundled, checksummed inventory of 128 known
exposed recordings with the selected parent and sibling `gate-*` cohort manifests.
The bundle's [provenance](../../scripts/research_directions/source_scaling/known-exposure.json)
records the original five inventories, so the historical CSVs need not be copied
from other project directories. Exposure coverage remains relative to these
records; this does not establish that every other recording was never inspected.

Before the first submission, additional inventories can be supplied as a
colon-separated list of CSV paths with a `video_id` column:

```bash
export FI_INSPECTED_MANIFESTS="/absolute/path/earlier-study.csv:/absolute/path/another-study.csv"
```

For known participant links, supply a CSV with `video_id,participant_id` columns:

```bash
export FI_PARTICIPANT_REGISTRY="/absolute/path/participants.csv"
```

The new launcher snapshots these inputs, its own source files and its software
identity under `$FI_RUN_ROOT/launch/`; the frozen configuration binds the
initialization record. Completed calibration is reused. Interrupted calibration
attempts are preserved and restarted in a new attempt directory. Freezing is
staged and published by directory so initialization can recover after a crash.
...
The launcher is under `slurm/source-scaling/` and delegates to jobs
...
`compute` needs completed poses; `fit` needs the completed plan; `report` needs
all required fits. `verify` reconstructs completed results on CPU without writing
scientific artifacts. Data and model job dependencies are `afterok`. Requested resources
are four CPUs/64 GB/24 hours for initialization; the allocations for jobs 20–23
remain those listed below. These are limits, not measured runtime estimates.

## Executed source learning-curve notebooks

The registered source-study notebook is
[`notebooks/source_scaling/23_source_learning_curves.ipynb`](../../notebooks/source_scaling/23_source_learning_curves.ipynb).
The launcher modes `all`, `compute`, `fit` and `report` append CPU job
`inspect-notebooks.sbatch` after their production jobs. Its `afterany`
dependency names every job in that submission, so an early report cancellation
cannot start inspection while other submitted fits are still running. `prepare`
does not append a notebook job. Use `submit.sh notebooks` to inspect preparation
or rerun the notebook independently once the jobs you want to inspect have settled.

Notebook-only submission requires `GAVD6_ROOT`, `FI_RUN_ROOT` and the existing
Python environment. It needs no parent-cache override, raw videos, teacher
checkpoint or GPU. Job 24 requests two CPUs, 8 GB RAM and one hour. For direct
execution outside Slurm, from the repository root:

```bash
.venv/bin/python scripts/research_directions/source_scaling/inspect_notebooks.py \
  --run-root "$FI_RUN_ROOT"
```

The runner starts a fresh kernel using its own Python interpreter, explicitly
passes `FI_RUN_ROOT`, and checks that the source notebook matches its canonical
generator. `--timeout` sets a per-code-cell limit in seconds (default 600).
...
during execution cause failure rather than an apparently stable inspection.
These hashes identify what was read; they do not replace integrity checks on
models, cache contents, or numerical reconstruction. An incomplete study can
produce a successfully executed notebook, and a complete scientific STOP can
also produce one. Neither notebook success nor failure changes the saved
scientific decision. The synthetic-report fixture used in tests is explicitly
labeled and is not real experimental evidence.

## Manual execution and relocation
...
The [main HAIC guide](../future-prediction/README.md) covers the historical gate commands. Read the
...
[validation record](../../docs/studies/future-feature-prediction/scaling/implementation.md)
...
Use the existing compatible environment. The [environment setup section](../future-prediction/README.md#prepare-the-environment-and-inputs)
...
```bash
"$FI_PYTHON" scripts/research_directions/source_scaling/run.py status \
...
```bash
bash slurm/source-scaling/submit-stages.sh all --dry-run
```

For an unattended run, submit every stage with its dependencies:

```bash
bash slurm/source-scaling/submit-stages.sh all
```

Alternatively, submit preparation separately:

```bash
bash slurm/source-scaling/submit-stages.sh prepare
```
...
```bash
bash slurm/source-scaling/submit-stages.sh compute
```
...
[notebook 23](../../notebooks/source_scaling/23_source_learning_curves.ipynb). For a completed report, check
its numerical results independently:

```bash
"$FI_PYTHON" scripts/research_directions/source_scaling/run.py verify \
  --run-root "$FI_SCALING_ROOT" \
  --parent-root "$FI_PARENT_ROOT"
```
...
```bash
bash slurm/source-scaling/submit-stages.sh fit
```
...
"$FI_PYTHON" scripts/research_directions/source_scaling/run.py plan \
  --run-root "$FI_SCALING_ROOT" \
  --parent-root "$FI_PARENT_ROOT"

"$FI_PYTHON" scripts/research_directions/source_scaling/run.py run \
  --run-root "$FI_SCALING_ROOT" \
  --parent-root "$FI_PARENT_ROOT"

"$FI_PYTHON" scripts/research_directions/source_scaling/run.py report \
  --run-root "$FI_SCALING_ROOT" \
  --parent-root "$FI_PARENT_ROOT"
...
`python -m gavd6_sjepa.research_directions.source_scaling.cli`.
...
"$FI_PYTHON" scripts/research_directions/source_scaling/calibrate.py \
...
```bash
"$FI_PYTHON" scripts/research_directions/source_scaling/run.py freeze \
  --run-root "$FI_SCALING_ROOT" \
  --parent-root "$FI_PARENT_ROOT" \
...
[validation record](../../docs/studies/future-feature-prediction/scaling/implementation.md#concrete-blocker-and-the-next-executable-step)
