...
We performed 2 audits to test confounding factors. The first checks a specific shortcut. The pose detector does not always find every landmark, and a joint that is occluded or off screen is marked invalid. Because each condition comes from only a few source videos, and some videos are harder to track than others, the pattern of detector failures can become correlated with the label. A model could then appear to read gait while actually reading which landmarks tend to disappear. To measure this shortcut, we discard all coordinates and keep only the validity mask for each sequence. We average it over frames to get the visible fraction of each of the 33 joints, and over joints to get the visible fraction within each frame. This forms a missingness signature that holds no geometry. We then fit the same Random Forest on the same split using this signature in place of the frozen embedding, so the input is the only thing that changes. A high score would not prove that the embedding uses the shortcut, but it would make a gait-only reading unsafe. The second audit counts source videos shared between train and test and test sequences whose source video appeared during self-supervised pretraining. Metrics include accuracy, balanced accuracy, macro F1, confusion matrices, and one-versus-normal area under the receiver operating characteristic curve (ROC AUC).
...
On the exact 47/21 split, frozen S-JEPA embeddings reach 0.619 accuracy, 0.596 balanced accuracy, and 0.613 macro F1. The saved 82-feature reference reaches 0.762 accuracy and 0.728 macro F1 on the same sequence identifiers. The accuracy difference is 0.143. The majority-class and missingness-only controls reach 0.294 and 0.333 accuracy; missingness macro F1 is 0.336. Because 0.333 sits just above the 0.294 majority baseline, missing pose patterns do carry some label-associated information. Because it sits far below the 0.619 of the embeddings, that shortcut does not explain the S-JEPA result.
