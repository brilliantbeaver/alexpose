...
\title{Learning Monocular Gait Representations through Neurologically Guided Skeleton JEPA}

\author{
\IEEEauthorblockN{Alex Mui, Penny Inouye, Theodore Mui, and Phil Mui}
\IEEEauthorblockA{\textit{Aspiring Scholars Directed Research Program (ASDRP)}\\
\texttt{alexander.mui@students.asdrp.org} \quad \texttt{pennypenelopesf@gmail.com}\\
\texttt{theodoremui@gmail.com} \quad \texttt{phil.mui@asdrp.org}}
}
...
\begin{IEEEkeywords}
gait analysis, self-supervised learning, S-JEPA, world models, neurologically guided masking, pose estimation
\end{IEEEkeywords}

\section{Introduction}
Walking reflects the combined action of the nervous system, muscles, joints, and balance control. Parkinson's disease often changes speed, stride length, swing time, cadence, and double-support time \cite{zanardi2021parkinsons}. Stroke commonly creates spatial and temporal differences between the paretic and non-paretic sides \cite{lauziere2014stroke}. Cerebral palsy can produce crouch gait with excessive knee flexion during stance \cite{pandey2023crouch}. Myopathies are heterogeneous. In late-onset Pompe disease, a systematic review found mainly symmetric hip and lumbar weakness, altered pelvic motion, and reduced locomotor function \cite{maulet2023myopathy}. These patterns make gait analysis useful, but gait alone does not establish a diagnosis.
...
Joint-Embedding Predictive Architectures (JEPAs) learn by predicting hidden content in representation space rather than reconstructing every input value \cite{assran2023ijepa}. S-JEPA applies this principle to skeleton sequences and reports strong generic action recognition \cite{abdelfattah2024sjepa}. Yet generic actions and clinical gait are different. High motion can define a kick, while reduced motion can be important in gait. Temporal resizing can also weaken absolute cadence information.
...
\begin{enumerate}
    \item a gait-specific S-JEPA mask restricted to 10 literature-linked shoulder, hip, knee, ankle, and foot landmarks;
    \item an end-to-end, provenance-checked pipeline from GAVD annotations to pose, pretraining, frozen embeddings, and classification;
    \item matched-sequence comparison with the 82-feature reference and explicit controls for missing pose data; and
    \item a source-video leakage audit that defines which claims the present experiment can and cannot support.
\end{enumerate}

\section{Related Work and Clinical Basis}
\subsection{Masked skeleton representation learning}
MAMP showed that predicting joint motion is more effective than reconstructing joint coordinates in its matched skeleton action experiments \cite{mao2023mamp}. It also made high-motion tokens more likely to be masked. S-JEPA keeps a related mask but changes the target. A view encoder sees only visible tokens, while a slowly updated target encoder sees the complete sequence. A predictor estimates the target encoder's latent vectors at hidden locations \cite{abdelfattah2024sjepa}. Skeleton2vec independently used contextual teacher targets for skeleton learning \cite{xu2024skeleton2vec}. Therefore, ``JEPA for skeletons'' is not the novelty of this work. Our narrower question is whether this method and a clinically motivated mask are useful for monocular gait.

Self-supervised clinical gait learning also predates this study. GaitForeMer used motion forecasting for few-shot impairment severity estimation \cite{endo2022gaitforemer}, and FSGait learned fine-grained abnormality representations \cite{duan2024fsgait}. Normal-only learning is therefore not new by itself. The distinct combination studied here is S-JEPA-style latent prediction on GAVD-derived monocular pose with a literature-constrained mask and explicit shortcut audits.

\subsection{Literature-guided landmark selection}
...
\subsection{Cohort and pose extraction}
GAVD contains 1,874 clinically annotated gait sequences from more than 450 public online videos \cite{ranjan2025gavd}. We lock this experiment to 96 sequences: 12 normal, 9 Parkinson's disease, 12 stroke, 16 cerebral palsy, and 47 myopathic. These sequences come from 18 source videos, with strong class-to-video concentration (Table~\ref{tab:cohort}).

\begin{table}[t]
\caption{Locked cohort and source concentration}
\label{tab:cohort}
\centering
\small
\begin{tabular}{lrr}
...
Each GAVD bounding box is used to crop its source video. MediaPipe Pose Landmarker then estimates 33 image-relative $(x,y,z)$ landmarks and visibility per frame \cite{grishchenko2022blazepose}. The resulting depth is a monocular model estimate, not calibrated clinical 3D motion capture. Marker-free MediaPipe gait measurements can agree well with motion capture for several temporal variables, but weaker agreement has been reported for some swing and double-support measurements \cite{hii2023markerfree}. Extraction runs in video mode with a CPU delegate and records the source video, frame numbers, crop, model hash, and extraction version.
...
\caption{The audited pipeline. The handcrafted and S-JEPA paths are a system-level comparison because their feature extraction differs.}
\label{fig:pipeline}
\end{figure}

\subsection{Gait-adapted S-JEPA}
...
The target encoder sees the complete sequence; the view encoder sees augmented unmasked tokens. Define $\mathbf{q}=\operatorname{softmax}((\mathbf{z}_t-\mathbf{c})/\tau_t)$ and $\mathbf{r}=\operatorname{softmax}(\mathbf{p}/\tau_p)$, where $\mathbf{c}$ is the EMA target-feature center that removes persistent channel bias, $\tau_t=0.06$, and $\tau_p=0.10$. For width $D=96$,
\begin{equation}
...
\subsection{Frozen downstream evaluation}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/training_health.pdf}
\caption{S-JEPA training and representation-health diagnostics.}
\label{fig:training}
\end{figure}

\subsection{Five-class classification}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.83\columnwidth]{figures/exact_split_results.pdf}
\caption{Exact-split accuracy. The 0.20 line is nominal five-class chance, not an uncertainty bound.}
\label{fig:results}
\end{figure}
