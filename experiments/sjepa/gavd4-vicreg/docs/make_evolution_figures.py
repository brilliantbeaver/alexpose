...
def connector(ax, start, end, color=None, width=1.2, dashed=False):
    ax.add_patch(
        FancyArrowPatch(
            start,
            end,
...
def make_timeline() -> None:
    ...
    for i, (heading, body, face, status, status_color) in enumerate(cards):
        ...
        if i < len(cards) - 1:
            connector(ax, (xs[i] + width + 0.003, 0.50), (xs[i + 1] - 0.005, 0.50))
    ax.text(
        0.02,
        0.08,
...
def make_layer_matrix() -> None:
    ...
    columns = [
        (0.02, 0.12, "Layer"),
        (0.15, 0.19, "Legacy prototype"),
        (0.35, 0.31, "Current completed run"),
        (0.67, 0.31, "Impact and remaining limit"),
    ]
    ...
    rows = [
        ...
        (
            ...
            "One continuing 600-epoch model\nnormal anchor still drifted to 0.594",
        ),
        (
            "Artifacts",
...
def make_masking_math() -> None:
    ...
    connector(ax, (0.305, 0.51), (0.352, 0.51))
    connector(ax, (0.645, 0.51), (0.692, 0.51))
    ax.text(
        0.02,
        0.08,
...
def make_augmentation_pipeline() -> None:
    ...
    for x, heading, body, face in top:
        card(ax, (x, 0.52, 0.20, 0.28), heading, body, face, heading_size=6.4, body_size=5.25)
    for x, heading, body, face in bottom:
        card(ax, (x, 0.14, 0.20, 0.28), heading, body, face, heading_size=6.4, body_size=5.25)
    for left, right in zip(top[:-1], top[1:]):
        connector(ax, (left[0] + 0.205, 0.66), (right[0] - 0.007, 0.66))
    connector(ax, (0.87, 0.515), (0.87, 0.43))
    for right, left in zip(bottom[:-1], bottom[1:]):
        connector(ax, (right[0] - 0.005, 0.28), (left[0] + 0.207, 0.28))
    ax.text(
        0.02,
        0.045,
...
def make_objective() -> None:
    ...
    connector(ax, (0.32, 0.64), (0.39, 0.64))
    card(
        ax,
        (0.40, 0.46, 0.58, 0.36),
        "Current objective",
...
def make_artifact_contract() -> None:
    ...
    nodes = [
        (0.02, 0.48, 0.16, "Notebook 04\ncheckpoint", "model + teacher\n5 completed stages", "#FFF0D6"),
        (0.22, 0.48, 0.16, "Lineage contract", "cohort + mask + loss\nparent fingerprint", "#F2EAF7"),
        (0.42, 0.48, 0.16, "Notebook 05\nembeddings", "96 canonical + 63 added\n384 values each", "#E4F2E8"),
        (0.62, 0.48, 0.16, "Notebook 06\nreadouts", "metrics + overlap\nmissingness controls", "#E4EEF7"),
        (0.82, 0.48, 0.16, "Figures and docs", "ledger values checked\nagainst artifacts", "#EDF0F2"),
    ]
    for i, (x, y, w, heading, body, face) in enumerate(nodes):
        card(ax, (x, y, w, 0.30), heading, body, face, heading_size=6.7, body_size=5.5, center=True)
        if i < len(nodes) - 1:
            connector(ax, (x + w + 0.003, 0.63), (nodes[i + 1][0] - 0.004, 0.63))
    checks = [
        "complete curriculum",
        "correct stage order",
        "12-target whitelist",
    ...
    x_positions = np.linspace(0.03, 0.77, len(checks))
    for x, label in zip(x_positions, checks):
        pill(ax, x, 0.25, label, COLORS["green"], width=0.17, text_size=4.8)
    ax.text(
        0.02,
        0.11,
...
def make_checkpoint_lineage() -> None:
    ...
    for stage, (x, metadata, condition) in enumerate(zip(xs, CHECKPOINT_METADATA, names)):
        ...
        if stage < 4:
            connector(ax, (x + width + 0.003, 0.52), (xs[stage + 1] - 0.006, 0.52))
    ax.text(
        0.02,
        0.115,
...
def make_classwise_results() -> None:
    ...
    fig.text(
        0.5,
        0.055,
        "Support below each class is A1 / A2. A2 macro-F1 is 0.742, yet stroke F1 is only 0.333.",
        ha="center",
        fontsize=6.2,
        color=COLORS["red"],
    )
...
def make_lane_c_repair() -> None:
    ...
    connector(ax, (0.41, 0.57), (0.58, 0.57), color=COLORS["blue"], width=1.5)
    ax.text(0.495, 0.625, "same encoder", ha="center", fontsize=5.6, color=COLORS["blue"])
    ax.text(0.495, 0.585, "new folds", ha="center", fontsize=5.6, color=COLORS["blue"])
    card(
        ax,
        (0.60, 0.32, 0.38, 0.50),
    ...
    ax.text(
        0.02,
        0.18,
        "Why only two folds? Parkinson's and cerebral palsy have two source videos each. Two is the largest fold count that keeps every class on both sides.",
        fontsize=6.0,
        color=COLORS["ink"],
    )
    ax.text(
        0.02,
        0.095,
        "What did not improve: the encoder was trained once on all 159 rows. Classifier-level video grouping does not create an independent encoder test.",
        fontsize=6.0,
        color=COLORS["red"],
    )
...
def make_readout_evolution() -> None:
    ...
    connector(ax, (0.25, 0.63), (0.31, 0.63))
    card(
        ax,
        (0.32, 0.45, 0.28, 0.36),
        "Current pooling",
    ...
    connector(ax, (0.61, 0.63), (0.67, 0.63), color=COLORS["purple"], dashed=True)
    card(
        ax,
        (0.68, 0.45, 0.30, 0.36),
    ...
    pill(ax, 0.755, 0.415, "PROPOSED", COLORS["purple"], width=0.15, text_size=5.2)
    card(
        ax,
        (0.02, 0.07, 0.46, 0.25),
