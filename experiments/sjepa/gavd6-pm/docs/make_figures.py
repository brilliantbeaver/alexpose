...
from artifact_paths import ROOT, load_contract, resolve_artifacts  # noqa: E402
...
def rounded_box(ax, xywh, text, facecolor, fontsize=7, textcolor=None, pad=0.008) -> None:
    """Draw a rounded card whose visual edge sits ``pad`` outside the given rectangle.

    The pad used to be 0.012, which pushed neighbouring cards to within a couple of points of
    each other and left no room for a connector between them.
    """
    x, y, w, h = xywh
    ax.add_patch(
        FancyBboxPatch(
...
def arrow(ax, start, end, color=None, dashed=False) -> None:
    """Draw one connector in the shared style, from and to the nominal card edges.

    shrinkA and shrinkB in ARROW_STYLE pull both ends back from the given points, so passing
    the nominal card rectangle edges is what produces clear space at the arrowhead.
    """
...
def make_pipeline() -> None:
    ...
    # This is a pure diagram with no visible axes, so it takes the whole canvas. On the default
    # subplot margins the drawing area was only 990 of 1278 pixels wide, and the 288 pixels of
    # wasted margin were exactly what the connectors between cards needed.
    ...
    # The top row is laid out from measured copy widths: each card is only as wide as its
    # longest line needs, and the slack is spent on the gaps so the connectors have room.
    # The wider labels are wrapped to three short lines rather than shortened.
    boxes = [
        (0.010, 0.58, 0.120, 0.25, "GAVD video\nand frame rows", "#E6F0F7"),
        (0.180, 0.58, 0.120, 0.25, "33-landmark\npose sequence", "#E6F0F7"),
        (0.350, 0.58, 0.132, 0.25, "12 landmark IDs\neligible for\nmasking", "#F2EAF7"),
        (0.532, 0.58, 0.140, 0.25, "Normal first, then\n4 cumulative\nstages", "#FFF0D6"),
        (0.722, 0.58, 0.120, 0.25, "Frozen 384-D\nreadout vector", "#E4F2E8"),
        (0.892, 0.58, 0.098, 0.25, "RF probes\nand audits", "#E4F2E8"),
        (0.336, 0.12, 0.160, 0.22, "Uniform target draw\nno motion score", "#F8E4DA"),
        (0.517, 0.12, 0.170, 0.22, "JEPA + VICReg\n+ group after Stage 0", "#F8E4DA"),
        (
            0.710,
            0.12,
            0.280,
    ...
    # Straight vertical risers into the card centres above, never a diagonal that would cross
    # a card border on its way in.
    for bottom, top in ((boxes[6], boxes[2]), (boxes[7], boxes[3]), (boxes[8], boxes[5])):
        x = top[0] + top[2] / 2
        arrow(ax, (x, bottom[1] + bottom[3]), (x, top[1]))
    ax.text(
        0.01,
        0.94,
...
def make_cohort_curriculum() -> None:
    # The right panel carries five stage cards plus four connectors, so it needs the wider
    # share of the row for the connectors to be visible rather than pinched.
    fig, axes = plt.subplots(1, 2, figsize=(7.6, 2.65), gridspec_kw={"width_ratios": [0.72, 1.58]})
    canonical = pd.Series(
    ...
    canonical_normal = int(canonical["Normal"])
    added_normal = 63
    axes[0].barh(canonical.index[::-1], canonical.values[::-1], color=colors[::-1])
    axes[0].barh(
        ["Normal"],
        [added_normal],
        left=[canonical_normal],
        color="#B7D6B0",
        hatch="///",
        edgecolor=COLORS["green"],
    )
    # Derive the limit and both label positions from the data so a cohort change cannot
    # silently push a label off the bar or off the axes.
    normal_total = canonical_normal + added_normal
    axes[0].set_xlim(0, max(int(canonical.max()), normal_total) * 1.09)
    axes[0].set_xlabel("Sequences")
    axes[0].set_title("Training corpus: 159 sequences")
    axes[0].grid(axis="x", alpha=0.25)
    normal_row = len(canonical) - 1
    axes[0].text(
        canonical_normal / 2,
        normal_row,
        str(canonical_normal),
        va="center",
        ha="center",
        fontsize=6.5,
        color="white",
    ...
    axes[0].text(
        canonical_normal + added_normal / 2,
        normal_row,
        f"+ {added_normal} project-labeled normal",
        va="center",
        ha="center",
        fontsize=6.1,
    )
    ...
    stages = [
        ...
        # Wrapped rather than shortened: the full label does not fit one line in this card.
        ("4", "Cerebral\npalsy", "75 epochs", COLORS["gold"]),
    ]
    # Narrow cards on purpose: the slack goes to the gaps, which is where the connectors live.
    # At 0.145 wide the gaps left a four-point shaft that was entirely arrowhead.
    stage_width = 0.115
    stage_gap = (0.96 - len(stages) * stage_width) / (len(stages) - 1)
    xs = 0.02 + np.arange(len(stages)) * (stage_width + stage_gap)
    for index, (stage, label, epochs, color) in enumerate(stages):
        rounded_box(
            ax,
            (xs[index], 0.40, stage_width, 0.32),
            f"Stage {stage}\n{label}\n{epochs}",
            color,
            fontsize=5.5,
        )
        if index < len(stages) - 1:
            arrow(ax, (xs[index] + stage_width, 0.56), (xs[index + 1], 0.56))
    ...
    # A little extra left margin keeps the longest y tick label inside the canvas rather than
    # relying on the tight bounding box to rescue it.
    fig.subplots_adjust(left=0.14, right=0.99, bottom=0.31, wspace=0.22)
...
# Condition names as drawn. The x-axis copy is wrapped instead of rotated: rotated labels of
# this length sat within a few pixels of each other, and every line here is narrower than one
# heat-map column, which is what keeps them provably separated.
CONDITION_LABELS = ["Normal", "Parkinson's", "Stroke", "Myopathic", "Cerebral palsy"]
CONDITION_TICKS = ["Normal", "Parkinson's", "Stroke", "Myopathic", "Cerebral\npalsy"]
...
def make_representation_geometry() -> None:
    ...
    ax.set_xticks(range(5), CONDITION_TICKS, rotation=0, fontsize=6.5)
    ax.set_yticks(range(5), CONDITION_LABELS)
    # Extra title pad keeps the heading clear of the colour bar's topmost tick label.
    ax.set_title("Canonical 96: cosine distance between condition centroids", pad=10)
    ...
    fig.text(0.5, 0.012, note, ha="center", va="bottom", fontsize=6.7, color=COLORS["muted"], linespacing=1.5)
    fig.tight_layout(rect=[0, 0.10, 1, 1])
    save(fig, "representation_geometry")


def make_exact_split_confusion() -> None:
    """Redraw the exact-split confusion matrix as a vector figure.

    The slide deck used to embed the notebook's PNG straight from the artifact
    directory, which broke whenever the active artifact root moved. Rebuilding it here
    keeps the deck inside the repository and under the same lineage validation.
    """
    matrix = pd.read_csv(ARTIFACTS / "five_class_confusion_matrix.csv", index_col=0)
    report = pd.read_csv(ARTIFACTS / "five_class_classification_report.csv", index_col=0)
    pretty = {
    ...
    # Same wrapping rule as the geometry heat map, so the two figures agree.
    wrapped = {"Cerebral palsy": "Cerebral\npalsy"}
    labels = [pretty.get(name, name) for name in matrix.index]
    tick_labels = [wrapped.get(label, label) for label in labels]
    values = matrix.to_numpy(dtype=float)
    fig, ax = plt.subplots(figsize=(4.5, 3.6))
    image = ax.imshow(values, cmap="YlGnBu", vmin=0, vmax=values.max())
    for row in range(values.shape[0]):
        for column in range(values.shape[1]):
            ...
            ax.text(
                ...
                va="center",
                fontsize=8,
                color="white" if count > values.max() * 0.6 else COLORS["ink"],
            )
    ax.set_xticks(range(len(labels)), tick_labels, rotation=0, fontsize=6.5)
    ax.set_yticks(range(len(labels)), labels)
    ax.set_xlabel("Predicted condition")
    ax.set_ylabel("Annotated condition")
    ax.set_title("Exact 47/21 split: confusion counts", pad=10)
    fig.colorbar(image, ax=ax, fraction=0.047, pad=0.04, label="Sequences")
    correct = int(values.trace())
    total = int(values.sum())
    # Two lines: on one line this note was wider than the figure and only survived the crop.
    fig.text(
        0.5,
        0.012,
        f"{correct} of {total} test sequences correct, accuracy "
        f"{float(report.loc['accuracy', 'f1-score']):.3f}.\n"
        "Videos cross this split and the encoder already saw every row.",
        ha="center",
        va="bottom",
        fontsize=6.7,
        color=COLORS["muted"],
    ...
    fig.tight_layout(rect=[0, 0.10, 1, 1])
    save(fig, "exact_split_confusion")
...
def make_readout_results() -> None:
    ...
    axes[0].set_title("Sequence-split accuracy and controls", pad=18)
    # The legend sits above the data area. Inside the axes it landed on the 0.857 bar label.
    axes[0].legend(
        loc="lower center",
    ...
    fig.subplots_adjust(left=0.08, right=0.98, bottom=0.31, top=0.72, wspace=0.20)
...
def make_result_changes() -> None:
    ...
    axes[0].set_title("Model revision on the exact exp5 split", pad=18)
    # Both legends moved above the data area; inside the axes they collided with bar labels.
    axes[0].legend(loc="lower left", bbox_to_anchor=(0.0, 1.005), ncol=2, frameon=False, fontsize=6.6)
    ...
    axes[1].set_title("Evaluation repair with the same checkpoint", pad=18)
    axes[1].legend(loc="lower left", bbox_to_anchor=(0.0, 1.005), ncol=2, frameon=False, fontsize=6.6)
    ...
    fig.subplots_adjust(left=0.08, right=0.98, bottom=0.24, top=0.74, wspace=0.22)
...
def make_evidence_ladder() -> None:
    ...
    for x, title, body, badge, face, badge_color in cards:
        ...
        ax.text(x + 0.145, 0.30, badge, ha="center", va="center", fontsize=6.2, color="white", fontweight="bold")
    # Two lines: on one line this note ran to within a few pixels of the canvas edge.
    ax.text(
        0.02,
        0.09,
        "Independent performance still requires fold-local preprocessing choices\n"
        "and fold-local retraining of all five representation stages.",
        va="top",
        fontsize=6.8,
        color=COLORS["red"],
        linespacing=1.5,
