...
Therefore `group 0.0008` means the squared centroid-margin shortfall was small on average across pairs and balanced batches. It cannot be converted into one exact distance. `std 0.4134` means EMA-teacher features retained nonzero spread on average. That example line comes from the end of Stage 1, and the final Stage 4 value is 0.363. It is not the VICReg variance hinge, is not optimized directly, and has no target of 1. Both values can look healthy while representations still encode background, video identity, pose-estimation artifacts, or other nuisance signals.
...
That optional flip is worth naming precisely, because a later experiment reuses it. Reflecting the body and swapping left and right landmark identities is the anatomical mirror operator $M$ that Idea 9 arm 2 applies to build a label-free training signal: it passes a sequence and its mirrored copy through the encoder and penalizes the two readouts for failing to be negatives of each other. The two uses point in opposite directions. As a `geometric_view` augmentation, the flip would tell the model that a body and its mirror are the *same* input, which discards side information that is clinically meaningful, so it stays off here. As arm 2's training term, the same operation instead asks the encoder to represent the mirrored body as the *negation* of the original, which preserves side information rather than discarding it. See Section 15 of `../staged_details.md` for that experiment, its endpoint, and its preregistered verdict.
...
Nonzero feature spread is evidence against total collapse and nothing more. On the completed run, the final mean feature standard deviation was 0.363, which is well above zero, so the sequences did not all map to the same vector. That number still does not identify what information the representation uses.
...
Report the geometry result on the **canonical** cohort, meaning the fixed 96-sequence GAVD set, with its measured numbers rather than with an adjective. Notebook 05 measures a cosine silhouette of 0.054 and a closest pair of condition centroids only 0.026 apart, while the mean distance inside a single condition is 0.104. A silhouette near zero means the condition clouds overlap instead of sitting apart, and a centroid gap smaller than the spread inside one condition means the two nearest classes, myopathic and cerebral palsy, are not separated.

What that evidence supports is narrow. Training completed, and the features stayed varied at a feature spread of 0.363, but five clean condition groups did not form on this cohort. What it does not support is any claim about which signal the surviving variation carries. These numbers are equally compatible with stable features that respond to **nuisance variables** such as camera, acquisition path, extraction route, or source video rather than to the intended gait properties. Model architecture, optimization completion, representation geometry, and clinical validity are separate claims and should remain separate in documentation.

The valid test of generalization is not a better number on this cohort. It is outer source-video retraining: hold out complete source videos first, choose preprocessing and train all five stages using only the outer training videos, then open the sealed videos once and score them.
