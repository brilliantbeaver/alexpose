...
- **Keep horizontal flip OFF** — it swaps left/right and destroys pathology laterality.
  - FORWARD POINTER (added 2026-08-19, this guardrail is still correct). The same anatomical mirror was
    later used deliberately, not as an augmentation but as a TRAINING SIGNAL, in Idea 9 Arm 2. The
    distinction matters and is the reason this rule survives. As an augmentation, a flip tells the
    encoder that a left-weak walk and a right-weak walk are the same thing, which erases exactly the
    laterality we care about. As a training signal, the mirror is applied to a copy and the encoder is
    asked to NEGATE its signed readout on that copy, which teaches the sign convention instead of
    erasing it. `flip_probability` stayed 0.0 in every arm. What we observed: the label-free mirror
    residual rho, on a scale where 0 is mirror equivariant and 4 is mirror blind, fell from a control
    mean of 0.462 to 0.059 on 18 of 18 source videos, using 3 seeds run against 5 registered. What we
    may conclude: the mirror term does teach the sign convention, but the run earned NO CREDIT because
    the preregistered `feature_std` guardrail regressed 0.0288 against a seed spread of 0.0082, so
    representation collapse remains a competing explanation. See
