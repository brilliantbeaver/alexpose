...
Older extraction code wrote the augmentation cohort below an experiment-local
``cache/artifacts/real`` directory (notably ``gavd4-vicreg`` and ``gavd5``)
even when notebook 04 was configured to read a different
``GAVD_ARTIFACT_DIR``. This utility discovers those known locations and migrates
only the archives selected by the legacy extraction report. It is deliberately
guarded:
...
def choose_legacy_source_root(candidates: list[Path]) -> Path:
    """Return the first candidate containing both the report and pose directory."""
    for candidate in candidates:
        candidate = candidate.expanduser()
        if (
            (candidate / "augmented_pose_extraction_report.csv").is_file()
            and (candidate / "poses_augmented" / "normal").is_dir()
        ):
            return candidate
    return candidates[0].expanduser()


configured_legacy_root = os.getenv("GAVD_LEGACY_REAL_ARTIFACT_DIR")
LEGACY_SOURCE_CANDIDATES = (
    [Path(configured_legacy_root)]
    if configured_legacy_root
    else [
        TUTORIAL_DIR / "cache" / "artifacts" / "real",
        PROJECT_ROOT / "experiments" / "sjepa" / "gavd4-vicreg"
        / "cache" / "artifacts" / "real",
        PROJECT_ROOT / "experiments" / "sjepa" / "gavd5"
        / "cache" / "artifacts" / "real",
    ]
)
DEFAULT_LEGACY_SOURCE_ROOT = choose_legacy_source_root(LEGACY_SOURCE_CANDIDATES)
...
def main() -> None:
    ...
    parser.add_argument(
        "--source-root", type=Path, default=DEFAULT_LEGACY_SOURCE_ROOT,
        help=(
            "legacy real-mode artifact root (default: GAVD_LEGACY_REAL_ARTIFACT_DIR "
            "or the first complete known experiment-local cohort)"
        ),
    )
    parser.add_argument(
