...
**The internal cohort (the only data you actually need).** This is the gavd5 GAVD cohort (Ranjan et al.,
...
1. **Pick one model and write down its name.** Two saved copies of the trained model exist locally: the
   augmented curriculum-final one with fingerprint `ea59fea0`, and a canonical one with fingerprint `dba24a`.
   A "fingerprint" is a short code that names exactly one saved copy. Comparing numbers made by two different
   copies would be like weighing things on two different scales, so you must pin one. Use `ea59fea0`. This
   step is zero-retrain.

2. **Re-run every clip through that one model.** Take the cached 528-token tensors for all 96 canonical
   sequences and all 63 augmented-normal windows, and pass them through the `ea59fea0` model to get each
   ...
   it. On every embedding row, record four things: the fingerprint (`ea59fea0`), the source-video id, the
...
4. **Set up three simple classifiers (three "lanes").** Each lane is an L2-regularized logistic regression,
   ...
   - Lane 1, the embedding: the frozen `ea59fea0` fingerprint of each clip.
...
- **One-fingerprint binding.** Every row is embedded by the single `ea59fea0` model, so no result mixes two
  model lineages.
...
- **One model, named on every row.** Bind everything to fingerprint `ea59fea0`, and record that fingerprint,
...
chance) with the source-permutation null band, all under the one `ea59fea0` fingerprint. How to read it: each
