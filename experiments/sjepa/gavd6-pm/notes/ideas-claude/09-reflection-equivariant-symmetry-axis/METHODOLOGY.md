...
## STATUS: this is a PROPOSAL document, and its decision rule has been SUPERSEDED

Read this before you use anything on this page as a rule or as a result.

This guide describes how to run the idea. It was written before the experiments ran, so every threshold
and every worked example on this page is a PLAN, and the worked examples are labelled "illustrative
numbers only" because that is exactly what they are. Two things have changed since.

First, **the decision rule in section 5 is superseded.** Its three gates (beat the standard encoder by
0.05 R-squared, reach at least 80 percent of the raw-coordinate null, and get the decoded sign right on
at least 75 percent of held-out sources) were replaced before any real run. The 80-percent gate is
unreachable because the raw-coordinate null sits near 1.0 by near-circular construction, and a fixed
75-percent sign threshold is meaningless when per-condition source counts are 1, 2, 3, 10, and 2. The
seven HARDENED gates that the code actually enforces are: a binding bar against `max(D, C)`, beat the
floor, a capacity-matched attribution control, a permutation null, an ABSOLUTE nuisance bound
`abs(E) < 0.05` on a mirror-symmetrized side-blind lane E, a y-quality gate on the target's
between-source variance fraction, and the exact wiring identity. Do not score any real number with
section 5's gates. See [IMPLEMENTATION.md](./IMPLEMENTATION.md) section 5 for the authoritative rule.

Second, **the experiments have run, and the outcomes are recorded.** Idea 05 returned an INFORMATIVE
NULL, Arm 1 returned ARTIFACT (side-agnostic nuisance control fired), and Arm 2 returned NO CREDIT.
Those three verdicts mean three different things and are explained side by side in
[IMPLEMENTATION.md](./IMPLEMENTATION.md) section 9a, with the register of retired claims in section 9b.
The single most important measured fact is about the cohort rather than the model: only 7.5 percent of
the labelled target's variance lies between the 18 source videos, against a preregistered 30 percent,
measured in `nb_09a`.

CHECKPOINT NOTE. The standard encoder on this page, in every method step, comparator, endpoint, and
gate, is the curriculum-final target encoder whose fingerprint prefix is `ea59fea0`, and every measured
number is bound to it. An earlier `d0acc262` checkpoint, same configuration on a differently extracted
pose cache, is SUPERSEDED; it is named here once, as history, so a stale number is recognizable.
...
(myopathy) better than the standard `ea59fea0` model that was only allowed to learn that behavior,
...
1. Build the antisymmetric head (needs new code, no training yet). Take the six left-right joint
   ...
   sides go through the identical box and only their difference survives, swapping the left and right
   joints at the head's input is forced to flip the total's sign. In symbols, with `f` the shared box
   and `L_k`, `R_k` the two joints of pair `k`, the axis is `s = sum over k of ( f(L_k) - f(R_k) )`,
   and swapping turns every `f(L_k) - f(R_k)` into `f(R_k) - f(L_k)`, its negative. Only the summed
   DIFFERENCE goes into `s`; there is no `left + right` term, because a `left + right` sum does not
   change under the swap (it is mirror-invariant) and would break the guaranteed sign-flip.

2. Prove the guarantee before trusting anything (zero training). There are two different mirror
   checks, and it matters which one the "exactly -1" claim belongs to.
   - The WIRING check (exact). Take the encoder's per-joint features, then at the head's input swap
     each left joint's feature with its right partner's, and check that the head's reading flipped
     sign to floating-point precision (slope exactly -1, to about 1e-6). This is an algebra fact
     about the head alone: `s = sum ( f(L_k) - f(R_k) )` becomes `sum ( f(R_k) - f(L_k) ) = -s` the
     moment you swap the inputs, no matter what the encoder did. This is the check you must pass
     before trusting anything; if it fails, stop and fix the wiring.
   - The ANATOMICAL-mirror check (measured, approximate). Take the raw skeleton, apply the true
     anatomical mirror (negate the sideways coordinate AND swap left/right landmarks), run it back
     THROUGH the encoder, then read the head. Because the encoder mixes all tokens with attention and
     adds a learned position code to each joint, the mirrored skeleton's features are NOT a clean
     swap of the originals, so the slope here is a measured number, not -1. For the standard
     `ea59fea0` model item 05 measured -0.741 (it does not flip, because the flip band is -1.25 to
     -0.8). We report this measured slope honestly and never call it -1.
   See ./images/fig2.svg. How to read that picture: if the model is an honest left-minus-right scale,
   mirroring the person flips the reading, so all dots should sit on the diagonal running top-left to
   bottom-right; the wiring check sits exactly on it by algebra, while the anatomical-mirror slope is
   only as close to it as the encoder happens to be.

3. Keep the backbone the same (no change). The S-JEPA backbone stays as it is: one small Transformer,
   4 heads, GELU, pre-norm. Never hardcode its size; always read the shape from the saved checkpoint's
   `config` block and build the model with `SJEPAGait(**checkpoint["config"])`. The real `ea59fea0`
   checkpoint on disk is embed_dim 96, encoder depth 4, predictor depth 2 (the smoke checkpoint is a
   smaller embed_dim 32, depth 1 stand-in). Only the readout head is new.

4. Retrain the full curriculum (this is the expensive step, it DOES need retraining). Match the
   `ea59fea0` lineage exactly: Stage 0 trains on normal for 300 epochs, then Stages 1 to 4 add
   ...
   `ea59fea0` model, so the only difference between them is the wiring, not the training data.

5. Watch for collapse during training (a health check, no extra retrain). Track feature spread
   (standard deviation), effective rank, and mean pairwise cosine against the `ea59fea0` reference
   figures. The `ea59fea0` model finished at feature standard deviation 0.362567 and mean pairwise cosine
   0.659870, so use those as sanity landmarks. "Collapse" means the model got lazy and made every
...
6. Cache the fingerprints (zero retrain from here on). For each sequence, save the 528-token features
   from the retrained equivariant model, and separately from the standard `ea59fea0` model. From now
...
8. Run item 05's mirror-slope check on every model (zero retrain). Apply the exact anatomical
   left-right mirror to the raw skeleton, re-embed THROUGH the encoder, decode, and fit the slope of
   decoded-mirrored versus decoded-original against the ideal line y = -x. This slope is a MEASURED,
   approximate number for every model, including the built-in head, because the encoder mixes tokens
   and adds per-joint position codes (the standard `ea59fea0` model measured -0.741). The only
   place the slope is exactly -1 is the separate wiring check in step 2, where the swap happens at the
   head's input instead of through the encoder. Do not confuse the two: the wiring check confirms the
   head is built correctly; the anatomical-mirror slope reports how close the whole model comes.
...
model MINUS that of the standard `ea59fea0` model, both read with item 05's identical frozen probe.
...
Suppose on 4 held-out source videos the standard `ea59fea0` probe scores R-squared 0.36, the
...
- One-fingerprint binding. Bind every number to a single checkpoint before comparing. The standard
  baseline is the `ea59fea0` lineage; the equivariant model gets its own recorded fingerprint. Do not
...
- Fix all seeds so runs repeat, and remember that changing the seed is not the same as changing the
  source video.
- Bind every result to ONE checkpoint fingerprint before comparing: `ea59fea0` for the standard
...
- Signed axis: one number that can be positive or negative; the sign says which side leans, the size
  says how much.
- Reflection-equivariant (antisymmetric by construction): swapping the left and right joints at the
  head's input is guaranteed by the wiring to flip the output's sign, so `s(swap of x) = -s(x)` for
  every input, trained or not. This exact guarantee is about the head's own algebra (the step-2
  wiring check). The full anatomical mirror runs through the encoder first, so its measured slope is
  only approximately -1, not exactly -1.
