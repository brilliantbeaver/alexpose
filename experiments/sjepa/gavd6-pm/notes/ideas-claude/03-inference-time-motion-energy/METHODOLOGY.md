...
1. **Load the one frozen model and check its fingerprint.** Load the encoder, the target encoder, and the
   predictor from the curriculum-final checkpoint whose fingerprint prefix is `ea59fea0`. A second lineage
...
8. **Package everything bound to `ea59fea0`.** Save the estimator code, the split manifest, the per-source
...
- **One-fingerprint binding.** Bind every number to the single checkpoint `ea59fea0` and refuse the `dba24a`
...
- **One checkpoint only.** Bind every number to fingerprint `ea59fea0`, verify it on load, and refuse the
