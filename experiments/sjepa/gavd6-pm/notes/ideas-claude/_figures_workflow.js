...
encoder (warm #e07a4b) vs the standard ea59fea0 encoder (blue #2f6f99). Draw the pre-registered +0.05 R-squared
advantage as a shaded band / annotation. Right: lane legend + pass card (dark banner) stating "by-construction
must beat ea59fea0 by >= 0.05 held-out-source R-squared".`,
...
EXACTLY on y = -x (guaranteed negation; warm dots tight on the line, label "guaranteed"); the learned ea59fea0
...
Encoder, Compute budget, Updates, Mask, showing IDENTICAL values across all four rows (e.g. same embed 96 /
depth 4, same 11,400 updates, same 12-joint mask) with a checkmark strip confirming "only the target differs".
