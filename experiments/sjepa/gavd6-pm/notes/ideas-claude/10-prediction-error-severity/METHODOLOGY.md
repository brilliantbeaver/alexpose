...
count for the core: one (the normal-only world model). This idea does NOT reuse the project's `ea59fea0` curriculum
...
3. Train the world model at project scale (the one retrain). Reuse the existing 33-joint tokenization, 528 tokens,
   embedding dimension 96, encoder depth 4, 4 heads, and the same masking pipeline (configured target 0.60 of the eligible
...
4. Bind the new checkpoint to its own fingerprint (hygiene). Give the resulting model its own fingerprint and keep it
   strictly separate from the project's `ea59fea0` curriculum checkpoint and the observed `dba24a` lineage. Mixing
...
- ONE FINGERPRINT. Bind every number to the new normal-only checkpoint's own fingerprint, kept separate from `ea59fea0`
...
- Monocular capture. gavd5 is single-view, so no genuine multi-view or out-of-plane claim is possible.
...
- One checkpoint, its own fingerprint. Train the normal-only world model once, bind it to a fresh fingerprint, and keep it
  strictly separate from `ea59fea0` and `dba24a`. Every reported number cites that one fingerprint.
