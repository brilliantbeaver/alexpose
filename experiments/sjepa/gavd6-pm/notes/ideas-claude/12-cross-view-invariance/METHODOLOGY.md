...
that touches the internal gavd5 data is transductive, which means the model was trained on the very clips we later test
...
This is not a random worry inside the project either. The gavd5 training pipeline already sets its flip chance to 0.0 by
...
Here is the honest scope. gavd5 is the project's own dataset of monocular (single-camera) YouTube walking clips, so it
...
### 3.2 The secondary data: the gavd5 GAVD cohort (internal, transductive side-check)
...
myopathic 10, cerebral palsy 2, and all 12 normal sequences come from one video). gavd5 enters ONLY as a small secondary
mirror probe on the already-frozen `ea59fea0` checkpoint. Because that encoder saw those rows and the source video is the
independent unit, every gavd5 number is labeled transductive.
...
gavd5 uses MediaPipe BlazePose with 33 joints (Grishchenko et al. 2022, arXiv:2206.11678). The public cohorts use their
...
Reading the math (why 528 tokens on the gavd5 side): each gavd5 sequence is resized to 64 frames, 4 adjacent frames form
...
because the whole idea is a new predictor that conditions on `view_delta`. The gavd5 secondary probe is ZERO-retrain: it
just reads the already-frozen `ea59fea0` features. Where the existing project tooling fits, we reuse it: the 528-token
joint-time tensor layout, the frozen `ea59fea0` checkpoint for the side-check, and the existing predictor scale (a
...
7. Run the gavd5 secondary probe (zero-retrain). On the frozen `ea59fea0` features, run the same mirror probe and check
   both its slope and whether it reaches 80 percent of the Lane E raw null. Label every gavd5 number transductive.
...
- ONE fingerprint. The gavd5 secondary probe binds to the single `ea59fea0` checkpoint before any comparison, so the
  `dba24a`-versus-`ea59fea0` lineage difference cannot sneak in as a fake effect. A "fingerprint" is just a short code
...
- gavd5 stays secondary and transductive. Every gavd5 number is labeled transductive, the encoder saw the rows, 18 source
...
| D gavd5 secondary probe | Frozen `ea59fea0` per-token features | No | Within-dataset transductive side-check |
...
- Transductive gavd5. The gavd5 side-check uses an encoder that saw every gavd5 row during training, so no gavd5 number
  is a fresh-people estimate. The source video is the independent unit, and there are only 18 of them.
- Tiny, lopsided internal sample. gavd5 has as few as one source video per condition and all 12 normal clips from one
  video, so it can only ever be a small secondary probe, never the headline.
- Provenance confound on gavd5. Most normal rows use an augmented extraction path and every abnormal row uses the
  canonical path, so a within-gavd5 comparison could learn processing differences instead of walking.
- Monocular capture inside gavd5. gavd5 is single-view, which is the whole reason the core invariance arm has to borrow
...
- One checkpoint, one binding. Pin the gavd5 secondary probe to the single `ea59fea0` fingerprint before any comparison,
...
- Fix the seeds. Train Lane A and Lane B under identical seeds and budgets so the only difference is the named factor.
  ...
  values.
- Label every number. Mark every public-cohort number as method-only and non-clinical, and every gavd5 number as
...
The gavd5 condition folder labels (normal, parkinsons, stroke, myopathic, cerebral_palsy) are dataset annotations from
GAVD (Ranjan et al., IEEE Access 2025, DOI 10.1109/ACCESS.2025.3545787), not diagnoses made by this project, and every
gavd5 result is transductive and small-sample with the source video as the independent unit. The signed-asymmetry scalar
