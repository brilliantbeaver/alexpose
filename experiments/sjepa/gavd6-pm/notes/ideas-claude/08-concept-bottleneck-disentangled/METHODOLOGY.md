...
- EFFECTIVE RANK: a health check on the fingerprint. It counts how many of the 96 numbers are really pulling their own
...
features this model learned mash all three of those things into one undifferentiated list of 96 numbers per token. An
...
by magic. See [`./images/fig3.svg`](./images/fig3.svg) for the plain picture of the whole idea (the 96-number fingerprint
...
portfolio). We keep the S-JEPA shape the same (33 joints times 16 time positions, embed_dim 96, encoder depth 4, 4 heads) and the
...
1. Fix the block layout (zero-retrain, do it first and write it down). Set aside three back-to-back slices of the 96-number
   ...
   ```python
   # Fixed block layout inside the 96-number embedding (logged before training).
...
- ONE fingerprint. The new run gets its own written-down fingerprint. The original curriculum-final checkpoint prefix
  `ea59fea0` (and the observed canonical lineage prefix `dba24a`) are used only as the Lane E ablation (the original model
...
| E original ea59fea0 (no named heads) | Frozen curriculum-final features | No | Ablation | Entangled: steering one axis drags the others |
...
- Monocular capture. gavd5 is single-view (one camera), which bounds how well side-view posture angles can be recovered.
...
- Bind to ONE fingerprint. The augmented retrain logs its own written-down fingerprint. The original `ea59fea0` (and the
  canonical lineage prefix `dba24a`) appear only as the Lane E ablation, and every number is tied to one fingerprint
  before any comparison.
- Fix the seeds, and keep the same shape as the original run (embed_dim 96, encoder depth 4, 4 heads) so the retrain differs from
