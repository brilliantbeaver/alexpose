...
**The internal work (the whole core study) uses one dataset: the gavd5 GAVD cohort** (Ranjan et al.,
...
on coordinates, mostly on a normal laptop CPU. The frozen checkpoint (fingerprint `ea59fea0`) is only
...
8. **(Optional, last week.) Overlay the frozen model as labeled context only.** If you want, you can
   show how the pooled model behaves on resized input, bound to checkpoint `ea59fea0`, clearly marked as
...
- **One-fingerprint binding.** Any optional model overlay is bound to checkpoint `ea59fea0` and never
...
- **Transductive framing.** Every gavd5 readout is transductive: the encoder saw every evaluation row
...
- **Provenance confound.** Most normal rows use the augmented extraction path while every abnormal row
  ...
  stating.
- **Monocular capture.** gavd5 is single-camera, so depth and true 3-D motion are estimated, not
  measured.
...
- **One checkpoint.** Bind any optional model overlay to the single fingerprint `ea59fea0`, never mixing
