...
- The final model is one frozen checkpoint with fingerprint prefix `ea59fea0`. Every number in these proposals is bound to that one checkpoint before any comparison. An earlier `d0acc262` checkpoint, same configuration on a differently extracted pose cache, is SUPERSEDED and must never be used as the current encoder.
...
## Two of these twelve have since been RUN. Read their outcomes before their proposals.

Everything in this folder was written as a proposal, and the plain questions in the table below are
written as open questions because that is how they were framed. Items 05 and 09 are no longer open.
Here is what they measured, so nobody reads a settled question as a live one.

- **Item 05, signed laterality decodability. Verdict: INFORMATIVE NULL.** We asked whether the frozen
  encoder makes a signed left-minus-right axis linearly readable. On the authoritative `ea59fea0`
  checkpoint the learned lane scored R-squared -0.602, the untrained-encoder floor -0.156, the
  side-blind pooled control -0.131, and the raw-coordinate null 1.000. Sign consistency was 44.4 percent
  against a required 75 percent, and the measured anatomical-mirror slope was -0.741, outside the flip
  band. The measurement was valid and the answer was no.
- **Item 09 Arm 1, the antisymmetric readout on the frozen encoder. Verdict: ARTIFACT (side-agnostic
  nuisance control fired).** A mirror-symmetrized side-blind lane, which cannot see left from right at
  all, scored -0.066 while the antisymmetric treatment scored -0.206. That withdraws the claim rather
  than answering it, which is a weaker epistemic state than a null.
- **Item 09 Arm 2, training the symmetry into the encoder. Verdict: NO CREDIT.** The label-free mirror
  residual rho, on a scale where 0 is mirror equivariant and 4 is mirror blind, fell from a control mean
  of 0.462 to 0.059 on 18 of 18 source videos. The effect is real and large. But the preregistered
  `feature_std` guardrail regressed 0.0288 against a seed spread of 0.0082, so representation collapse
  is a live competing explanation and the win cannot be credited to symmetry. The run used 3 seeds
  against 5 registered, and the grouped five-class guardrail was not evaluable on this cohort.

Those three verdicts mean three different things and must not be collapsed into "it did not work". The
binding constraint behind all three is the COHORT rather than the model or the readout: only 7.5 percent
of the labelled target's variance lies between the 18 source videos, against a preregistered 30 percent,
which was measured in `nb_09a` (Arm 1) and nowhere else. Everything here is transductive, and none of it
is a clinical claim. The full comparison and the register of superseded claims are in
...
The proposals span four themes: evaluation validity, world-model / predictive, mechanism / design, and a fourth theme, neuro-grounded world model, for the biomarker-anchored items 08 to 12 (items 05 and 09 also carry a mechanism reading). The table is sorted by scorecard composite, highest first. Those composites are PROPOSAL-QUALITY scores that grade the design of a study, not results from running it. The full graded rubric, with all five axes per item, is in [SCORECARD.md](./SCORECARD.md).
