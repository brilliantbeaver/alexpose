...
- Coordinate dim 3 (monocular BlazePose x, y, relative z). One token embeds a 4-frame x 3-coord = 12-vector
  via a linear layer to embed_dim 96; encoder depth 4, predictor depth 2, 4 heads, GELU, pre-norm.
...
- Five-stage curriculum: Stage 0 normal (300 epochs), Stages 1-4 add PD, stroke, myopathic, cerebral palsy
  (75 epochs each). 600 curriculum epochs, 11,400 optimizer updates. The authoritative curriculum-final
  target encoder has fingerprint prefix `ea59fea0`. Every current number in this file, and every current
  number in the twelve proposals, is bound to it. A canonical (non-augmented) lineage prefix `dba24a` has
  also been observed locally; bind every result to ONE fingerprint before comparing.
- CHECKPOINT SUPERSESSION, read this before quoting a fingerprint. An earlier `d0acc262` prefix appears in
  proposal-era drafts and in a leftover artifact bundle. It is SUPERSEDED: same configuration and same
  five-stage recipe, but a differently extracted pose cache. It is named here only so a stale number can be
  recognized on sight. Never use it as the current encoder, comparator, or baseline.
- Final feature standard deviation 0.362567 (not total collapse), mean pairwise cosine 0.659870.
- Normal-anchor cosine fell 0.959074 (after Stage 1) -> 0.617498 (after Stage 4): substantial drift.
- Superseded `d0acc262` figures, kept only so old prose is recognizable: feature standard deviation
  0.413745, mean pairwise cosine 0.609342, normal-anchor cosine 0.954 -> 0.594.

## Representation geometry (notebook 05, mean/std pooling to 384-d)
- A mean and a standard deviation are permutation-invariant, so this pooled readout discards temporal order
  by construction.
- Cosine silhouette 0.054398 (no clean five-group separation).
- Minimum centroid distance 0.025675 (smaller than mean within-condition distance 0.104404).
- Mean centroid distance 0.313160. Closest centroids: myopathic and cerebral palsy.
- Decoded gait scalars: step amplitude recovers well (R^2 ~0.719); ASYMMETRY is the weakest decodable
  scalar (R^2 ~0.154). Cycle-to-cycle variability (stride_time_cv) is not linearly decodable from ~2s windows.
  These two R-squared values are carried forward from the notebook-05 readout diagnostic and are NOT
  re-verifiable against the current `ea59fea0` artifact bundle, which ships no scalar-decodability table.
  Treat them as unverified context, not as current measured results. The signed-laterality question they
  motivated has since been measured directly, and the authoritative answer is Idea 05's informative null
  in `09-reflection-equivariant-symmetry-axis/IMPLEMENTATION.md`.
- Superseded `d0acc262` geometry, kept only so old prose is recognizable: cosine silhouette 0.008975,
  minimum centroid distance 0.036718, mean within-condition distance 0.119521, mean centroid distance
  0.292119.
...
- All-96 stratified S-JEPA: accuracy 0.759, balanced 0.849, macro-F1 0.803 (all test videos overlap
  training; every test row trained the encoder).
- All-96 missingness-only control: accuracy 0.483, balanced 0.507, macro-F1 0.477 (visibility only, no
  gait coordinates).
- Video-grouped binary readout, 5 folds over 159 sequences: accuracy 0.780, balanced 0.804, macro-F1 0.749,
  ROC-AUC 0.915 (probe folds group videos, but the encoder saw all 159 rows). Normal-vs-abnormal
  separability at the embedding level is high, and it is exactly the number the provenance confound puts
  at risk.
- Video-grouped five-class readout, 2 folds: accuracy 0.614, balanced 0.615, macro-F1 0.615; pooled
  out-of-fold 0.616, 0.613, 0.610 (encoder saw all rows).
- Superseded readout figures, kept only so old prose is recognizable: all-96 stratified 0.793, 0.889,
  0.821; missingness-only 0.448, 0.466, 0.429; video-grouped binary 0.849, 0.874 with about 0.96 AUC;
  video-grouped five-class 0.653 accuracy and 0.625 macro-F1. None of these are current.
- All readouts are descriptive and transductive. None is a clinical statement or an unseen-video result.
