...
Emits the notebook one directory up (the gavd5 root) so it sits beside 00-06.
...
It asks one falsifiable question on the frozen `ea59fea0` Skeleton-JEPA (S-JEPA) checkpoint:
...
- `GAVD_MODE=real`: reads the real pose cache and the `ea59fea0` checkpoint, exactly as notebooks
  05 and 06 do.
...
`load_state_dict` on the `ea59fea0` checkpoint matches key-for-key. We ALWAYS construct the model with
...
curriculum completion, conditions seen), verify the `ea59fea0` fingerprint prefix, and construct the
...
- Lane A (learned probe): flattened per-token features from the frozen `ea59fea0` target encoder over
