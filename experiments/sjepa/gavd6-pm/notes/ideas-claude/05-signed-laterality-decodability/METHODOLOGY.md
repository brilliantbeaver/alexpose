...
## STATUS: this is a PROPOSAL document, and the experiment has since run

Read this before you use anything on this page as a result.

This guide describes how to run Idea 5. Every threshold on this page is a PLAN, and every worked
example is labelled "illustrative numbers only, not measured facts" because that is exactly what it is.
Keep them labelled that way. But the experiment has been carried out, so here is the outcome in one
paragraph, so nobody mistakes a plan for an open question.

`nb_05a` ran on the frozen curriculum-final target encoder, fingerprint prefix `ea59fea0`, over 5
source-disjoint folds on 96 canonical sequences from 18 source videos. Lane A (learned) scored
R-squared -0.602, Lane B (raw-coordinate null) 1.000, Lane C (untrained floor) -0.156, and Lane D
(side-blind pooled control) -0.131. Sign consistency was 0.444, that is 44.4 percent, against a
required 75 percent. The measured anatomical-mirror slope was -0.741, outside the flip band, so the
scalar did not count as flipping. All three primary gates failed, and two controls outscored the
treatment: C by 0.446 and D by 0.471. **Verdict: INFORMATIVE NULL.** The measurement was valid and the
answer was no: the frozen representation does not make a signed laterality axis linearly available
above a raw-coordinate baseline, and does not even reach an untrained-encoder floor. It does not show
that no side information exists anywhere, because nonlinear readouts were never tested, and it is not a
clinical claim.

Three further things a reader needs.

1. **`nb_05b` produced no empirical verdict about the checkpoint.** It is a possible-futures simulator
   plus an external multi-view reach scaffold whose real loaders are still TODO. Its synthetic fixture
   carries a view-stability correlation near +1 that was PLANTED there by construction. Never cite its
   simulated futures or that fixture correlation as an empirical result.
2. **These gates were SUPERSEDED for Idea 9.** The section-5 rule below (beat the floor by 0.05, reach
   80 percent of the raw null, sign correct on 75 percent of sources) is what produced Idea 5's null and
   is correct for Idea 5. It must NOT be applied to Idea 9. Idea 9 replaced it with a binding bar
   against `max(D, C)`, a capacity-matched control, a permutation null, an absolute nuisance bound
   `abs(E) < 0.05` on a mirror-symmetrized side-blind lane E, a y-quality gate, and the exact wiring
   identity, and it dropped the 80-percent-of-null gate entirely because the raw null sits near 1.0 by
   near-circular construction.
3. **The 7.5 percent between-source variance measurement belongs to `nb_09a`**, Idea 9 Arm 1, not to
   Idea 5 and not to `nb_05b`. It says only about 7.5 percent of the labelled target's variance lies
   between the 18 source videos, against a preregistered 30 percent, which is why source-disjoint folds
   hold out nearly all the usable signal by construction and why the binding constraint is the cohort
   rather than the encoder.

The three verdicts in this family (Idea 5's informative null, Idea 9 Arm 1's artifact, Idea 9 Arm 2's
no credit) mean three DIFFERENT things and are compared side by side in
...
target encoder whose fingerprint prefix is `ea59fea0`. Every measured number above is bound to it. An
earlier `d0acc262` checkpoint used the same configuration on a differently extracted pose cache and is
SUPERSEDED; it is named here once, as history, so a reader can recognize a stale number on sight.
...
- EMBEDDING: a short "fingerprint of numbers" the model uses to describe a token. Here each token becomes a list of 96
  numbers.
...
1. Bind to ONE checkpoint (zero-retrain). Use the curriculum-final target encoder with fingerprint prefix `ea59fea0`.
   Two other lineages exist locally, a canonical prefix `dba24a` and a superseded `d0acc262` from an earlier pose
   extraction, and mixing any of them would be a confound, so pin every number to `ea59fea0` before any comparison. Load the checkpoint under the same guards as notebook 05 (correct mode, the
   ...
   checkpoint is a small Transformer, embed_dim 96, encoder depth 4, predictor depth 2, 4 heads). That way the weights match key for key
...
2. Reuse the cached 528-token tensors (zero-retrain). For each of the 96 canonical sequences, run the frozen target
   encoder over all 528 joint-time tokens (the target encoder always sees the full set) and cache the 96-number embedding
...
5. Fit the four lanes (zero-retrain, test-time only). Fit a ridge linear regression to predict the signed target. A linear
   ...
   |---|---|---|---|---|
   | A learned probe | Frozen `ea59fea0` per-token features, side-structured | No | Primary | Beat the floor by at least 0.05 R-squared and reach at least 80 percent of the null |
...
6. Run the mirror-equivariance test (zero-retrain). Apply the frozen mirror to each input, re-embed through the same
   frozen `ea59fea0` encoder, decode with the already-fit Lane A probe, and compare the decoded number on the original
...
- ONE fingerprint. Bind everything to `ea59fea0` before any comparison, so a `dba24a`-versus-`ea59fea0` lineage
...
Two notebooks carry the work. Their full paths, relative to this file, are given once here, and the
short names `nb_05a` and `nb_05b` are used everywhere after that:
...
They write two result files, named once here and afterwards called the Idea 5 result JSON and the
futures bundle:

~~~text
idea5_signed_laterality_result.json
idea5_futures_bundle.json
~~~

- [`nb_05a`](../../../nb_05a_signed_laterality_probe.ipynb) is the decisive probe: it
  copies the S-JEPA model classes verbatim so `load_state_dict` matches key for key, loads the `ea59fea0` checkpoint under
  the notebook-05 guards, caches the frozen target-encoder features, fits Lanes A through D with source-video-disjoint
  ridge probes, runs the mirror pass, applies the pre-registered margins, and writes the Idea 5 result
  JSON. It runs in `GAVD_MODE=real` (reads the real checkpoint and pose cache) and
...
- [`nb_05b`](../../../nb_05b_reflection_reach_and_futures.ipynb) is the
  possible-futures simulator (it writes the futures bundle and the futures figure) and the honestly-stubbed external
  multi-view reach scaffold. It measures nothing about any checkpoint. Its simulated futures are made-up
  numbers walked through the margins, and its synthetic multi-view fixture (18 clips, 6 subjects by 3
  views) returns a view-stability correlation near +1 only because that correlation was planted in the
  fixture. Neither may be reported as an empirical result.
- Determinism. Fix the seeds. The smoke lean overlay is deterministic. Save the frozen target function, the
  ...
  Idea 5 result JSON against the four canonical futures in the futures bundle, so anyone can
