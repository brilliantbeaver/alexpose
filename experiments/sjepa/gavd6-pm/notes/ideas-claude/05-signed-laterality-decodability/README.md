...
## STATUS: this page is the PROPOSAL. The experiment has since run. Here is what it measured.

Everything after this section is the original proposal, and its worked examples are labelled
"illustrative numbers only, not measured facts" because that is exactly what they are. Keep reading
them as a plan. But the experiment has been carried out, so a reader must not leave this page thinking
the question is still open.

WHAT WE DID. `nb_05a_signed_laterality_probe` fit the four-lane ridge readout described below on the
frozen curriculum-final target encoder, over 5 source-disjoint folds, on the canonical cohort of 96
sequences from 18 source videos. Nothing was retrained. Every number is bound to the authoritative
checkpoint whose fingerprint prefix is `ea59fea0`.

WHAT WE OBSERVED, in R-squared:

| Lane | Role | R-squared |
|---|---|---|
| B, raw-coordinate null | non-neural ceiling, descriptive only | 1.000 |
| A, learned probe on the frozen encoder | the treatment | -0.602 |
| C, untrained-encoder floor | the floor | -0.156 |
| D, mean and standard deviation pooled control | the side-blind nuisance lane | -0.131 |

All three preregistered gates failed. The learned lane did not beat the floor by 0.05; it came in
0.446 BELOW the floor. It did not reach 80 percent of the raw null. Sign consistency across held-out
sources was 0.444, that is 44.4 percent, against a required 75 percent. The measured anatomical-mirror
slope was -0.741, outside the flip band of -1.25 to -0.8, so the decoded scalar did not count as
flipping. Two controls outscored the treatment: the untrained floor C by 0.446 and the pooled control
D by 0.471.

WHAT WE MAY CONCLUDE. **Verdict: INFORMATIVE NULL.** On this cohort and under these gates, the frozen
representation does not make a signed laterality axis LINEARLY available above a raw-coordinate
baseline, and does not even reach an untrained-encoder floor. The measurement was valid and the answer
was no. What this does NOT show: that no side information exists anywhere in the representation, since
nonlinear readouts were never tested. It is not a clinical statement, and it is transductive, because
the encoder saw every evaluation sequence during training.

CHECKPOINT NOTE. The frozen encoder for this study, everywhere on this page, is the curriculum-final
target encoder whose fingerprint prefix is `ea59fea0`. An earlier `d0acc262` checkpoint used the same
configuration on a differently extracted pose cache and is SUPERSEDED. It is named here once, as
history, so that stale numbers can be recognized: any document quoting A = -0.187, C = +0.147,
D = -0.014, or a mirror slope of -0.343 as a current Idea 5 result is reading that superseded bundle
and is wrong.

### The second notebook, `nb_05b`, produced NO empirical verdict

This needs saying plainly, because the notebook looks like it produces results and does not. Its full
name is:
...
Short form `nb_05b` is used from here on. `nb_05b` is exactly two things: a possible-futures SIMULATOR
that walks made-up numbers through the decision margins on this page, and an external multi-view reach
SCAFFOLD (CASIA-B, OU-MVLP-Pose) whose real loaders are still marked TODO. Its only concrete numbers
come from a synthetic fixture of 18 clips, 6 subjects by 3 views, with a view-stability correlation near
+1 PLANTED in it by construction. Never cite `nb_05b`'s simulated futures or that near-unity fixture
correlation as an empirical result about anything.

### Where these proposal-level gates were SUPERSEDED

The three gates on this page (beat the floor by 0.05, reach 80 percent of the raw null, sign correct on
75 percent of sources) were used for Idea 5 itself and are the gates that produced the informative null
above. They were then REPLACED for Idea 9, and must not be applied to any Idea 9 number. Idea 9's
hardened gates are: a binding bar against `max(D, C)`, beat the floor, a capacity-matched control, a
permutation null, an ABSOLUTE nuisance bound `abs(E) < 0.05` on a mirror-symmetrized side-blind lane E,
a y-quality gate on the target's between-source variance fraction, and the exact wiring identity. The
80-percent-of-null gate was dropped entirely, because the raw null sits near 1.0 by near-circular
construction. See
[../09-reflection-equivariant-symmetry-axis/IMPLEMENTATION.md](../09-reflection-equivariant-symmetry-axis/IMPLEMENTATION.md)
section 5 for the rule and section 9a for how Idea 5's null, Idea 9 Arm 1's artifact, and Idea 9 Arm 2's
no-credit differ from one another.

### One attribution correction

The measurement that only about 7.5 percent of the signed-laterality target's variance lies BETWEEN
source videos, against a preregistered 30 percent, was made in `nb_09a` (Idea 9 Arm 1). It is not an
Idea 5 result and not an `nb_05b` result. It is the single most important number in this family,
because it says the binding constraint is the cohort of 18 source videos rather than the encoder or
the readout, and it explains Idea 5's null.
...
This project trained a self-supervised skeleton model (S-JEPA) on gait videos. "Self-supervised" means the model learned by predicting hidden parts of its own input, with no human labels. A separate analysis (notebook 05) tried to read simple gait numbers back out of the model's learned features. It found that asymmetry was the hardest one to recover. Step amplitude (how big each step is) came out with an R-squared around 0.719, but asymmetry sat at only about 0.154.
...
- 0.719 for step amplitude means the features explained about 72 percent of how step size varies across sequences.
- 0.154 for asymmetry means the features explained only about a sixth (about 15 percent) of how asymmetry varies.
...
Second, I run a mechanism test. If the features truly encode a SIGNED left-versus-right quantity, then physically swapping the left and right body landmarks (an anatomical mirror) should flip the sign of whatever I decode: a plus becomes a minus of the same size. I test that directly. That sign-flipping behavior has a name: an ANTISYMMETRIC encoding. In plain words, antisymmetric means that mirroring the input negates the output: if you feed in a mirrored body and the decoded number just changes sign (same size, opposite direction), the encoding is antisymmetric. Here is a homey analogy. Write a number on a piece of clear glass and hold it up to a mirror. If the reflection shows the same number with the opposite sign, the writing "respects" the mirror. We are asking whether the model's signed-asymmetry reading behaves like that: mirror the walker, and the number should cleanly flip. Testing this is the same as testing whether the encoder learned reflection-equivariance, which is a fancy way of saying a mirrored input produces a mirrored output. It is NOT an assumption that the encoder "understands anatomy." If the decoded value does not flip, that rules out a clean antisymmetric encoding. It tells us the asymmetry information, where it exists, is stored in some other way that a simple mirror does not neatly reverse.

## Why this matters

A positive result confirms a specific belief: the frozen S-JEPA features already contain a usable, signed, direction-carrying representation of gait asymmetry, and the disappointing 0.154 was a readout or measurement limit rather than an absence of the information. That would be good news. It means the clinically central axis is recoverable without retraining, just by reading the model differently.

A null result rules out an equally specific belief. If the signed axis does not clear the raw-coordinate baseline on held-out source videos, then we can stop blaming the model's weak asymmetry performance on pooling choices or probe capacity. The information is simply not linearly present in the tokens above what raw coordinates already give you. That directly explains the documented 0.154 and reframes it as a genuine representation limit. That is an informative negative result that reviewers value (ICLR/ICML/NeurIPS 2026 reward informative nulls that change understanding).
...
Each token turns a 4-frame by 3-coordinate (x, y, relative z) 12-vector into a 96-number embedding through a linear layer. A "12-vector" is just a list of 12 numbers: 4 frames times 3 coordinates each. A "96-dimensional embedding" is a list of 96 numbers the model uses to describe that token.
...
1. Bind to one checkpoint. Use the curriculum-final target-encoder checkpoint with fingerprint prefix `ea59fea0`. A canonical lineage prefix `dba24a` has also been observed locally, and a superseded `d0acc262` lineage exists from an earlier pose extraction. Every number in this study is bound to the single `ea59fea0` fingerprint before any comparison. Where a source-video-disjoint fold-local encoder checkpoint already exists, use it as a test-time-only pass and label honestly how much the encoder had already seen: if the encoder was trained on every row, mark the number as transductive with nothing truly held out from the encoder; if the encoder was trained without that fold's videos, mark the number as transductive only for that fold. Either way the label tells a reader exactly which rows the encoder had seen before the number was produced.

2. Reuse the 528-token target-encoder tensors. For each of the 96 canonical sequences, run the frozen target encoder on all 528 tokens (it always sees the full set) and cache the 96-dimensional per-token features, tagged with source video id, condition label, provenance (canonical vs augmented), checkpoint fingerprint, and encoder-exposure flag. These are the frozen artifacts every lane reads.
...
6. Run the mirror-equivariance test. Apply the exact `LEFT_RIGHT_PAIRS` mirror (negate x, swap left/right landmarks) to each input, re-embed through the same frozen `ea59fea0` encoder, decode with the already-fit probe, and compare the decoded scalar on original vs mirrored input against the line y = -x.
...
|---|---|---|---|---|
| A Learned probe | Frozen `ea59fea0` per-token features | No | Primary | Above floor by >= 0.05 R-squared, >= 80% of null |
...
- Bind to ONE fingerprint (`ea59fea0`) before any comparison, avoiding any cross-lineage confound with the canonical `dba24a` or the superseded `d0acc262` lineage.
...
This proposal is reified as two runnable notebooks plus a full methodology document, so the question above is not just described but executable. Nothing retrains the encoder; the primary arm is a test-time-only linear read of the frozen `ea59fea0` tokens.
...
- [nb_05a_signed_laterality_probe.ipynb](../../../nb_05a_signed_laterality_probe.ipynb): the decisive probe. It copies the S-JEPA model classes verbatim so the state dict matches key for key, loads the `ea59fea0` checkpoint under the same guards as notebook 05, caches the frozen target-encoder features, fits Lanes A through D with source-video-disjoint ridge probes (GroupKFold means the fold splitter keeps all clips from one source video together, so no source is split across train and test; the inner alpha, which is the ridge penalty size, is chosen using only the training sources of each fold so held-out sources never influence it), runs the anatomical-mirror equivariance pass, applies the pre-registered margins (beat the floor by at least 0.05 R-squared, reach at least 80 percent of the raw-coordinate null, decoded sign correct on at least 75 percent of held-out sources, mirror slope inside minus 1.25 to minus 0.8), and writes the Idea 5 result JSON. It runs in `GAVD_MODE=real` exactly as notebook 05 does and degrades gracefully to `GAVD_MODE=smoke`, which reuses the project synthetic fixtures plus one clearly-labelled signed lean overlay so the plumbing runs end to end. Smoke numbers are illustrative plumbing checks only. The result file it writes is:

~~~text
idea5_signed_laterality_result.json
~~~

- [nb_05b_reflection_reach_and_futures.ipynb](../../../nb_05b_reflection_reach_and_futures.ipynb): the pre-registration aid and reach scaffold. It produces NO empirical verdict about the checkpoint. It deterministically simulates the four canonical possible futures against the same margins and draws their expected shapes, renders one decision table mapping each future to its licensed claim (the two-by-two shape shown in Fig 4 below), writes `idea5_futures_bundle.json`, and lays down an honestly-stubbed loader interface for the non-clinical external multi-view reach arm (CASIA-B, OU-MVLP-Pose), exercised on a synthetic multi-view fixture of 18 clips, 6 subjects by 3 views. Wiring the real, large, licensed downloads is a marked TODO and is not done in this pass. The fixture's view-stability correlation comes back near +1 because it was PLANTED near +1 by construction, so it confirms the wiring and nothing else. Neither the simulated futures nor the fixture correlation may be cited as an empirical result.

First, the big idea in five steps, from a walking video to a signed number and a mirror check:

![The big idea in five steps: a walking video becomes a stick figure, the frozen model turns it into a fingerprint of numbers, a straight-line rule reads one signed left-minus-right number, and a mirror check flips the body and re-reads it](./images/fig3.svg)

Fig 3: The central idea as a five-step flow (illustrative numbers, not data). Step 1 is a short walking video. Step 2 turns it into a stick figure (a skeleton). Step 3 is the frozen S-JEPA model producing a fingerprint of numbers. Step 4 reads one signed left-minus-right number from that fingerprint (positive leans left, negative leans right). Step 5 is the mirror check: flip the body left-to-right and re-read, and a true signed number should come back the same size with the opposite sign. How to read this picture: follow the arrows left to right and top to bottom; each box does one small job, and the last box shows the hoped-for flip (+0.7 becoming -0.7).

The four possible futures and exactly what each one licenses:

![Two yes-or-no questions (does it read, does it flip) crossed into four honest outcomes: clean-flip positive, reads-but-does-not-flip, informative null, and side-blind-control-fires artifact](./images/fig4.svg)

Fig 4: The four canonical outcomes, laid out as two yes-or-no questions crossed into a two-by-two grid (simulated expected shapes, not data). Question 1 asks whether the signed number can be read and reaches 80 percent of the raw-coordinate baseline. Question 2 asks whether the number flips under the mirror. F1 clean-flip positive (reads yes, flips yes) licenses both the decodability and the reflection-equivariance claim; F2 reads-but-does-not-flip (reads yes, flips no) licenses decodability but withholds equivariance; F3 informative null (reads no) overturns the belief that the checkpoint organized a laterality axis and directly explains the weak recorded asymmetry score; F4 artifact (only the side-blind control fires) withdraws the signed claim. How to read this picture: pick the row for your read answer and the column for your flip answer, and the box you land on names the one honest claim that outcome allows. Given that asymmetry is the project's weakest-decoded scalar (R-squared about 0.154), F2 or F3 are the a priori more likely futures, and both are publishable.
