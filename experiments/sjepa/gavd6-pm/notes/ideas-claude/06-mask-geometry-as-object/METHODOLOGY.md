...
from 18 videos, an embedding width of 96, a 4-layer encoder) mask shape is not the lever, and effort should go
...
Internal work (the real study): the gavd5 GAVD cohort (Ranjan et al., IEEE Access 2025,
...
conservative conclusion stands: any clinical reading stays reach-tier and is never claimed from the 18 gavd5 videos.
...
1. Bind one lineage and one budget, but only the portable part. Take the shipped checkpoint fingerprint `ea59fea0` and
   copy only its recipe: the exact architecture (embedding width 96, encoder depth 4, 4 attention heads), the optimizer, the
...
- Permutation sanity check. This is a readout-time test, no retraining. Take the frozen Arm S encoder, run its mask so
  ...
  encoder.
- One-fingerprint binding. Bind every arm to the single `ea59fea0` recipe before comparing, so a `dba24a`-versus-
  `ea59fea0` lineage difference cannot masquerade as a mask effect.
- Feature-collapse check. Measure how spread out the features are (standard deviation, reference 0.362567 on the shipped
  run) and how similar they are on average (mean pairwise cosine, reference 0.659870). If an arm's features barely vary
...
- Arm S does not beat Arm U by the margin. Informative null. Licensed claim: at 96 sequences from 18 videos, an
  embedding width of 96, and a 4-layer encoder, mask shape is not the lever; look instead at data scale, token geometry,
...
- Monocular capture. gavd5 is single-view, so out-of-plane motion and transverse rotation are not seen.
...
- Bind one checkpoint recipe. Every arm inherits only the `ea59fea0` portable recipe (architecture, optimizer, update
