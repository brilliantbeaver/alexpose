...
fingerprint drifted a lot. Its cosine similarity to itself across stages fell from 0.959 after Stage 1 down
to 0.617 after Stage 4. Cosine similarity runs from -1 (opposite) through 0 (unrelated) to 1 (identical
direction), so 0.959 means the normal fingerprint barely moved early on, and 0.617 means it had shifted a
...
2. Bind everything to one fingerprint (no retraining). The reference encoder that saw everything is the
   five-stage, curriculum-final checkpoint with fingerprint prefix `ea59fea0` (600 curriculum epochs, 11,400
   optimizer updates). A second lineage prefix, `dba24a`, has also been seen locally; mixing them would be a
   confound, so bind every result to `ea59fea0` and print the fingerprint next to every number.
...
6. Measure separation two ways, on seen and on new videos (no retraining, just reading). For every fold and
   ...
   condition centers; the seen-video reference is 0.025675, which is small, matching the finding that
...
- Hard collapse gate for both arms: the feature standard deviation and effective rank must stay healthy
  (reference feature std 0.362567). A collapsed arm is rejected no matter how good its macro-F1 looks.
...
- One-fingerprint binding: bind every number to `ea59fea0` and print the fingerprint beside it, so no result
  quietly mixes the `ea59fea0` and `dba24a` lineages.
...
- Collapse gate: reject any arm whose features collapse (feature std far below 0.362567, or a low effective
...
- Bind to one checkpoint: `ea59fea0`, printed beside every number, never mixed with the `dba24a` lineage.
