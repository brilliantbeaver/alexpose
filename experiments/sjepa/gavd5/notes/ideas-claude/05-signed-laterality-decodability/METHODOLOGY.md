...
This matters for one concrete reason established in the project. In `_shared_facts.md`, the asymmetry
scalar is the WEAKEST decodable gait quantity from the pooled representation (R-squared about 0.154),
far below step amplitude (R-squared about 0.719). The mechanism prior predicts asymmetry should be the
MOST clinically load-bearing axis, yet the current mean/std-pooled readout decodes it worst. The most
likely culprit is that a mean and a standard deviation are permutation-invariant and side-agnostic by
construction, so they discard the very signed side information the axis needs. Idea 5 asks the sharper
question the pooled readout cannot: with a side-aware, signed target and a side-structured probe, is
the signed axis actually there in the token tensor, or did the encoder never organize one?
