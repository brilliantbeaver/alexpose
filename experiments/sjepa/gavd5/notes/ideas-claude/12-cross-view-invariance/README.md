...
- One fingerprint. The gavd5 secondary probe binds to the single `d0acc262` fingerprint before any comparison, avoiding the `dba24a`-vs-`d0acc262` lineage confound. A "fingerprint" here is just a short code that names one exact saved model, so we never accidentally mix two different training runs.
