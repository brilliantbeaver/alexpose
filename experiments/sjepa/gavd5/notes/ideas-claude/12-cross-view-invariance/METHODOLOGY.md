...
7. Run the gavd5 secondary probe (zero-retrain). On the frozen `d0acc262` features, run the same mirror probe and check
...
- ONE fingerprint. The gavd5 secondary probe binds to the single `d0acc262` checkpoint before any comparison, so the
...
| D gavd5 secondary probe | Frozen `d0acc262` per-token features | No | Within-dataset transductive side-check |
...
- One checkpoint, one binding. Pin the gavd5 secondary probe to the single `d0acc262` fingerprint before any comparison,
