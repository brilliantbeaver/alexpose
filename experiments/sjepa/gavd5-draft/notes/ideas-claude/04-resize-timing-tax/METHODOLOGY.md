...
**The internal work (the whole core study) uses one dataset: the gavd5-draft GAVD cohort** (Ranjan et al.,
...
- **Transductive framing.** Every gavd5-draft readout is transductive: the encoder saw every evaluation row
...
- **Provenance confound.** Most normal rows use the augmented extraction path while every abnormal row
  ...
  stating.
- **Monocular capture.** gavd5-draft is single-camera, so depth and true 3-D motion are estimated, not
  measured.
