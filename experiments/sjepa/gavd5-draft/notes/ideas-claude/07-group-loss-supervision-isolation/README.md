...
- **Regenerate artifacts inside gavd5-draft during Week 1.** No `.pt` or `pose_missingness_features.csv` exist in this clone. We re-extract `.npz` from CSVs and regenerate reference checkpoints and missingness features first, and gate the whole study on that reproduction (Step 0, Day-5 gate).
