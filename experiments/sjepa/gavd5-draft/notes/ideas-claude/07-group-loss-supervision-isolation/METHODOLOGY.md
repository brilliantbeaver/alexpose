...
The core runs entirely on the gavd5-draft GAVD cohort (Ranjan et al., IEEE Access 2025). No new data is collected.
...
5. Run a compact, fold-local fine-tune, not a full retrain (small retraining). Do NOT redo 11,400 updates
   ...
   weight (and, for OFF-random, the sampler) is allowed to differ. This retrains only at the small gavd5-draft
...
- Provenance confound. Normal is one video on a mostly-augmented path, and abnormal uses the canonical path.
  ...
  fully erase it.
- Monocular capture. gavd5-draft is single-view video, so it cannot answer view-stability questions; those belong
