...
The latest completed training checkpoint among `gavd4-vicreg`, `gavd5-draft`, and
`gavd5-draft-tm` is the augmented five-stage checkpoint under `gavd5-draft/work`. Its file
SHA-256 is:
...
From `experiments/sjepa/gavd5-draft`:
...
.venv/bin/jupyter nbconvert \
  --execute \
  --to notebook \
  --output /tmp/gavd5-draft-downstream-probe-reproduced.ipynb \
