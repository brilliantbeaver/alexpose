# NOTEBOOK nb_05b_reflection_reach_and_futures.ipynb
cells=20
...
2. **External multi-view reach scaffold.** The clinical claim on gavd5 (n=18 source videos) is
...
The gavd5 result is **transductive** because the encoder learned from the same corpus before its frozen
...
```
--- outputs (1) ---
[stream] IDEA5_DIR: /Users/pmui/dev/alexpose/experiments/sjepa/gavd5/notes/ideas-claude/05-signed-laterality-decodability
...
```
--- outputs (1) ---
[stream] scoring self-check OK (proposal worked example scores as a primary pass, mirror flips).
...
```
--- outputs (1) ---
[stream]                               future  A_r2  B_r2  C_r2  D_r2  sign_frac  mirror_slope                                                       primary                                             mirror  D_control_ok
...
F4 artifact (nuisance control fires)  0.47  0.50  0.06  0.46       0.82         -0.90                                 SIGNED AXIS PRESENT ABOVE RAW FLIPS (encoding is antisymmetric under the mirror)         False
...
### F3 informative null
  ...
  mirror  : DOES NOT FLIP (encoding is non-antisymmetric)
  D control ok: True
  claim   : LICENSED (negative): the frozen S-JEPA tokens do NOT add the signed axis above raw coordinates. Overturns the hypothesis that the checkpoint organized a laterality axis; a clean publishable negative for the representation audit.

### F4 artifact (nuisance control fires)
  primary : SIGNED AXIS PRESENT ABOVE RAW
  mirror  : FLIPS (encoding is antisymmetric under the mirror)
  D control ok: False
  claim   : WITHHELD: the signed claim is withdrawn. The side-agnostic mean/std control recovered the 'axis', so Lane A reflects a magnitude/acquisition a
...
```
--- outputs (2) ---
[stream] saved /Users/pmui/dev/alexpose/experiments/sjepa/gavd5/notes/ideas-claude/05-signed-laterality-decodability/images/idea5_possible_futures.png
...
The clinical claim on gavd5 is internal-validity only: n=18 canonical source videos, transductive
...
```
--- outputs (1) ---
[stream] external reach scaffold exercised on 18 synthetic multi-view clips (6 subjects x 3 views).
...
bundle = {
    ...
    "notes": "Simulated expected shapes, NOT data. gavd5 clinical claim is internal-validity only.",
}
...
```
--- outputs (1) ---
[stream] wrote /Users/pmui/dev/alexpose/experiments/sjepa/gavd5/notes/ideas-claude/05-signed-laterality-decodability/idea5_futures_bundle.json
{
  "margins": {
    "floor_margin": 0.05,
    "null_fraction": 0.8,
