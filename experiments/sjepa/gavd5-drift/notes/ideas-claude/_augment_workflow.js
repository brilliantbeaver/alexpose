...
const DIR = '/Users/pmui/dev/alexpose/experiments/sjepa/gavd5-drift/notes/ideas-claude'
...
- ${DIR}/_shared_facts.md   (single source of truth for every number and citation; NEVER contradict it)
- ${DIR}/_neuro_facts.md    (the discriminative symmetric-vs-lateralized axis, condition mechanisms, world-model
...
const ITEMS = [
  ...
  },
  {
    ...
    lift: `Promote from "does one structured mask beat uniform on gavd5-drift" to the MASK-FAMILY-AT-EQUAL-COVERAGE
  ...
  },
  {
    ...
    lift: `Viewpoint-conditioned predictor with view_delta as an "action" (V-JEPA-2 action-conditioning framing,
      ...
      gavd5-drift is monocular YouTube with no controlled multi-view data, so the CORE invariance arm runs on public
      multi-view POSE cohorts (CASIA-B Yu 2006; OU-MVLP-Pose Takemura 2018; GREW arXiv:2205.02692; Gait3D
      arXiv:2204.02569), all NON-clinical, with gavd5-drift view-labels only as a secondary probe; the clinical claim
...
- The generalizable-claim restatement (what transfers beyond gavd5-drift: the method/principle).
...
- overclaim_flags: HARD flag any clinical-accuracy claim on gavd5-drift (n=18 sources) that is not explicitly hedged as
...
- generalizability_verdict: does the result transfer beyond gavd5-drift (a method/principle), or is it only a gavd5-drift caveat?
...
WRITE two things:
1. ${DIR}/SCORECARD.md : a markdown file with (a) a short intro paragraph stating the rubric and the unifying
