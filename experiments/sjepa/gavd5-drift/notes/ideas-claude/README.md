...
- 12 treats viewpoint change as an action for a view-conditioned predictor with a strict no-flip rule, on external multi-view cohorts. No `plan/` item is cross-view or action-conditioned; `plan/07` stresses viewpoint only as a robustness sweep on gavd5-drift.
