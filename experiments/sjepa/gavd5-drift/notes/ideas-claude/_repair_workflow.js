...
- ${DIR}/_neuro_facts.md    (discriminative symmetric-vs-lateralized axis, condition mechanisms, world-model levers,
...
phase('Fix06')
const LIFT_06 = `Promote 06 from "does one structured mask beat uniform on gavd5-drift" to the MASK-FAMILY-AT-EQUAL-COVERAGE
...
two facts files. Return the review. HARD flag any clinical-accuracy claim on gavd5-drift (n=18) not hedged as external-
...
Then PATCH ${DIR}/SCORECARD.md IN PLACE: (a) update the item-06 row in the table with the new axes, composite, band,
...
TASK: Rewrite ${DIR}/_selection.json so its "selected" array holds all 12 items (currently only 7). Read the current
...
TASK: Rewrite ${DIR}/README.md as the portfolio index for all 12 items (currently it describes only seven). Read the
