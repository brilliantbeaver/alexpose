...
- EMBEDDING (also "latent" or "fingerprint" or "feature"): a short list of numbers the model uses to describe something.
  Instead of "hip is at these coordinates," the model can carry "hip summary = these 64 numbers." A latent is an internal
...
(notebook 05, summarized in [`../_shared_facts.md`](../_shared_facts.md)) already measured, on the frozen `d0acc262`
...
### 3.1 The main data: the gavd5-drift GAVD cohort (internal only)
...
1. Build the fixed substrate harness (setup, no retrain yet). Write ONE training config where the only field allowed to
   ...
   positions, 33 joints times 16 positions = 528 tokens, embed dim 64, depth 2, 4 heads, GELU, pre-norm; only 12
   ...
   11,400 optimizer updates matching the `d0acc262` curriculum; laterality flip OFF (flip probability 0.0); fixed seeds.
...
7. Monitor collapse per family (a health check, no retrain). For each of T1 to T4, report the final feature standard
   deviation and mean pairwise cosine. The reference healthy `d0acc262` checkpoint sat at feature std 0.413745 and mean
   pairwise cosine 0.609342, so a family whose std drifts far below that has quietly collapsed and its numbers are
   suspect.
...
- ONE-FINGERPRINT BINDING. Every retrain's budget matches the `d0acc262` curriculum and is bound to one lineage before
...
- COLLAPSE MONITORING PER FAMILY. Report feature std and mean pairwise cosine for each of T1 to T4 (reference healthy std
  0.413745, mean cosine 0.609342), since the latent-target families can collapse.
...
| A family collapsed | A latent-target family's feature std drifted far below 0.413745 | Result withdrawn for that family | Its recovery numbers are not trustworthy; the anti-collapse control caught a cheating shortcut |
...
- MONOCULAR CAPTURE. gavd5-drift is single-view, so out-of-plane (twisting) motion is not recoverable, and there is no
...
- ONE checkpoint lineage. Bind every retrain's budget to the `d0acc262` curriculum (5 stages, 600 epochs, 11,400 updates)
