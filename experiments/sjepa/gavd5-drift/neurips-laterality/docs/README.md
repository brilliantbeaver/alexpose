...
## FMTS 2026 revision and adversarial review record — updated 15 September 2026

This is a separate author-side FMTS revision series, starting editorially from Physical World AI V8 and reconciling it with the original paper, implementation, executed notebook outputs and retained numerical records. The original paper, Physical World AI files and their review history are preserved. The final manuscript is [FMTS V9](fmts_revisions/paper_v9.md), with its [PDF](fmts_revisions/paper_v9.pdf), [anonymous source package](fmts_revisions/paper_v9_overleaf.zip), [anonymous numerical supplement](fmts_revisions/paper_v9_supplement.zip), and [complete version index](fmts_revisions/README.md).
...
| FMTS V9 | 8.8 | 9 | 7.5 | 8 | 7 | 9.3 | 9 | 9.7 | **84.50** |
...
| Dimension | Score | Concrete reason, remaining weakness or next correction |
|:--|--:|:--|
| Temporal relevance | 8.8 | The abstract now begins with the FMTS evaluation problem and presents the gait study as a bounded test of a temporal representation. The target remains a whole-clip, time-reversal-invariant observable rather than a forecasting task. |
| Claim accuracy | 9 | The strategic abstract preserves the supported comparison without turning the paper into a numerical inventory. The same aggregate-only evidence boundary and readout scope remain explicit. |
| Evaluation/statistics | 7.5 | Source holdout, matched initialization, conditional intervals and leakage-aware readout selection are unchanged. No new participant metadata, ablation, multiplicity adjustment or external cohort was added. |
| Insight/positioning | 8 | The title and abstract make the distinction between hidden-feature matching and observable movement the paper's organizing idea. The mechanism and broader transfer still require experiments. |
| Reproducibility | 7 | A ninth private source package, numerical supplement and rendered record preserve the revision. Missing raw predictions, checkpoints and source-level bootstrap inputs remain unresolved. |
| Clarity/narrative | 9.3 | The abstract removes stage-by-stage counts and effect ranges, gives one headline comparison, and closes with the evaluation lesson. The full numerical evidence remains in Results. |
| Figures | 9 | V9 retains the verified V8 figures unchanged apart from their private version paths. Their labels, branches and print-size legibility remain strengths. |
| Submission fit | 9.7 | The title and abstract directly engage the CFP's evaluation, reliability, leakage-awareness and negative-results themes while avoiding claims of foundation-model scale or demonstrated forecasting. |
...
### Seven scored title suggestions

The title candidates use five equally weighted 0–10 criteria: objective accuracy, alignment with the FMTS evaluation and reliability themes, clarity, specificity, and restraint. The mean is an editorial comparison, not an acceptance score.

| # | Candidate title | Accuracy | FMTS | Clarity | Specificity | Restraint | Mean | Critique |
|:--|:--|--:|--:|--:|--:|--:|--:|:--|
| 1 | **Same-Clip Matching Improves While a Left-Right Movement Readout Worsens: A Gait S-JEPA Study** | 9.6 | 9.5 | 9.4 | 9.6 | 9.5 | **9.5** | Best result-led option. It names both outcomes and the bounded model setting, but needs the abstract to clarify that “improves” means a more consistent same-clip preference rather than directly comparable loss magnitudes across teacher spaces. |
| 2 | **Evaluating Predictive Gait Representations Beyond Latent-Feature Matching** | 9.0 | 9.8 | 9.7 | 8.1 | 9.8 | **9.3** | Selected for V9. It gives the paper the strongest workshop-facing frame by foregrounding evaluation of predictive temporal representations; the abstract supplies the empirical direction and left-right target that the title intentionally leaves out. |
| 3 | **A Gap Between Hidden-Feature Matching and Left-Right Movement Readout in a Gait S-JEPA** | 9.5 | 9.3 | 9.2 | 9.7 | 9.7 | **9.5** | Accurate and restrained, though “a gap” does not tell the reader which measure improves or worsens. |
| 4 | **Testing Observable Movement in Predictive Gait Representations** | 8.8 | 9.8 | 9.7 | 7.8 | 9.8 | **9.2** | Clean and strongly aligned with FMTS, but too broad to distinguish this study and silent about S-JEPA and laterality. |
| 5 | **S-JEPA Training Lowers Left-Right Movement Readout Accuracy Relative to Matched Initialization** | 9.8 | 8.7 | 9.3 | 9.9 | 9.5 | **9.4** | Most exact account of the main controlled comparison, but dense and incomplete because it omits the successful same-clip diagnostic. |
| 6 | **From Hidden-Feature Prediction to Movement Measurement: Evaluating a Gait S-JEPA** | 9.1 | 9.6 | 9.3 | 8.9 | 9.8 | **9.3** | Smoothly links pretraining to system measurement, but states the question rather than the negative finding. |
| 7 | **Clip Matching and Movement Readout Diverge After Gait S-JEPA Training** | 9.2 | 9.3 | 9.6 | 8.6 | 9.4 | **9.2** | Short and memorable, but it omits the left-right target and matched-initialization design. |

In plain language, the central result says: **After training, the model more consistently matches hidden features from the same walking clip. But a separate regression estimates the clip's left-right movement difference less accurately from the trained encoder's features than from its starting features.** Matching a clip's hidden features and estimating its movement are different evaluations; neither one tests prediction of future movement. V9 uses this distinction throughout, while defining the normalized median pose-speed contrast precisely in Methods.

### Precision and repeated-estimate audit

V9's abstract intentionally omits the stage-by-stage counts and score ranges. It states the direction of the two headline evaluations and leaves the numerical evidence in Results. The main text, table and plot use R² 0.223 versus 0.101–0.114. R² and interval limits generally use three decimals. Differences below 0.001 retain a sign and one significant digit: MAMP −0.0008 and mixture +0.0005. Their intervals remain visible, including the negative lower bound −0.015 after rounding the mixture's −0.015497…; no rounded zero is called equivalence. MAE uses four decimals (0.0415 to 0.0436–0.0440), with paired increases computed first (0.0020–0.0025). Percentages use one decimal when supported; integer counts and exact method settings are not rounded as estimates. Reflection summary values are rounded from the saved summary, without invented seed-level or bootstrap precision.
...
All nine manuscripts compile independently from their anonymous source ZIPs, and their extracted PDF text matches the delivered PDFs page for page. Every version has four main-text pages; V1–V7 have seven total pages, V8 has six, and V9 now has seven (four main, one references, two appendix). No margin or body-font reduction was used. The official style hash, manuscript/source/PDF hashes and private asset inventory are retained in each version's manifest.

At the completion of the nine-version review, all 61 rendered pages were checked in layout contact sheets, with larger inspections of the V1 and V5 figures and every final V8 and V9 page. The V9 source package was rebuilt in isolation and its pages inspected after the title and abstract revision. There are no clipped figures, overlapping labels, missing glyphs, unresolved citations or overfull boxes in the final artifact. V1–V4 retain their smaller figure lettering as part of the revision history; the review and lower figure scores identify that weakness. V5–V9 use larger labels, and V8 introduced explicit anatomical identities and the frozen encoder role. The schematic poses show neither participant images nor generated gait forecasts.

A subsequent V9 appendix revision organizes the implementation details and supporting analyses into five subsections across two pages. It clarifies missing-data handling, training settings, bilateral feature construction, source weighting and the limits of reproducibility. All reported results, the first five PDF pages and the figure files are preserved. Both revised appendix pages were inspected, and the updated source package compiled independently with matching text. The [appendix verification record](fmts_revisions/review/appendix_revision_v9_checks.json) identifies the current artifacts; earlier review records describe the versions inspected at those stages.

The [delivery checks](fmts_revisions/review/delivery_checks.json), [numerical check](fmts_revisions/review/evidence_check.json), [V9 review](fmts_revisions/review/review_v9_final.md) and version-specific rendered pages document the scope of verification. Source packages contain frozen manuscript TeX, bibliography, official style and vector figures; numerical supplements contain retained seed aggregates, recorded intervals, a mean/difference verifier and the corresponding figure generator. They intentionally contain no original videos, poses, prediction rows, checkpoints or identifying repository links. Thus the manuscript and figures are reproducible, while full experimental replication and source-bootstrap reconstruction still require the missing primary artifacts.
