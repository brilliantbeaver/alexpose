...
![Completed model. Poses and tokens are schematic: skeleton colors mark anatomical left/right; token colors mark visible (teal), hidden (orange) and missing (gray) positions. Scattered/motion-weighted and connected-region masks may leave context on both sides. Full clips reach the teacher and regularizer; target y supervises the separate readout.](assets/v7/figures/training_pipeline_compact.svg)

**Training.** The grid uses 1,200 AdamW updates per encoder (architecture and settings in Appendix A). The online encoder zeroes hidden patch content before adding joint and position embeddings; the predictor substitutes learned mask tokens at target positions. Cross-entropy matches centered teacher feature distributions at hidden valid positions, averaged within clips and then across clips. A teacher follows the online weights by exponential moving average without gradients. A full-view VICReg term [3], weighted 0.05, pools twelve gait joints, including hips, across two geometric views and passes them through a projector. Thus the total objective also accesses complete clips. Training uses CUDA BF16 and evaluation FP32.

We compare three motion conditions and two region conditions across five folds and five seeds: 125 encoders. The motion conditions use random sampling, a MAMP-style sampler [4], and a robust motion mixture. Motion masks sample across all 33 joints using a count derived from the twelve-joint gait pool. Connected regions hide six connected landmarks over eight of sixteen blocks, with a separate count-matched random reference. Each comparison matches initial weights, source draws, views and update counts. This grid uses neither reflection augmentation nor an explicit reflection loss (Appendix A distinguishes their evidence).
...
**Movement readout falls after training.** The expanded initial representation reaches mean $R^2=0.223$, compared with $0.101$–$0.114$ for trained teachers (Figure 2). All five conditions decrease in every seed. Mean absolute error rises by 0.0020–0.0025 in the dimensionless contrast, from an initial 0.0415 to 0.0436–0.0440. Paired trained-minus-initial intervals are below zero in every condition. Final online encoders also score below their matched initial encoders.
...
![Different outcomes from the same fitted models. A: frozen movement readout; points are five seed scores, and short vertical marks their mean. B: correct-clip feature error is lower in 375 of 375 trained diagnostic checks, versus 33 of 75 distinct initial checks. These checks reuse models and videos.](assets/v7/figures/learning_results.svg)
...
**Changing mask geometry does not establish better readout.** Inspection draws confirm higher-motion target selection. Connected regions reduce targets with both immediate temporal neighbors visible from 69.9% under matched random masking to zero. Motion masks hide approximately 17% of valid all-landmark tokens; regions hide 9.9%. Their readout comparisons therefore use distinct random references. All three mask-comparison intervals span zero (Appendix A). Their benefits remain uncertain; the intervals do not establish equivalence.
...
For a representation intended to support this gait measurement, the latent diagnostic gives an incomplete account of usefulness. The trained predictor favors the correct clip more consistently, yet the regression estimates its left–right contrast less accurately. Poorer readout performance does not imply complete destruction of movement information. In particular, 49/125 teacher fits and 96/125 online fits select the largest ridge penalty, 10,000. Wider training-only selection, support-only controls and summaries of matched dimension would help separate feature geometry from readout limitations. Source holdout does not establish participant independence, chronological holdout or an untouched development cohort. Recording and pose-estimation cues remain possible explanations.
...
Prediction targets and available context influence which variation a representation retains. Skeleton methods predict learned features [1] or explicit motion [4]; connected-body masks reduce nearby cues [5]. Action recognition evaluates the resulting features for labels, while video anticipation and action-conditioned planning assess other behavior [6]. Work separating invariant and transformation-sensitive features further exposes this dependence on the task [7]. Our gait contrast tests access to a chosen observable with matched initial weights. We borrow masking ideas without comparing the complete methods.
...
Archive coordinates are $p_x=(x_0+uW_c)/W$, $p_y=(y_0+vH_c)/H$, and $p_z=dW_c/W$, where crop origin and size are $(x_0,y_0),W_c,H_c$, frame size is $W,H$, and $u,v,d$ are pose estimates. The scale is the median of valid shoulder and hip widths. The movement path requires observed hips; the encoder uses the median valid pelvis midpoint for missing hips (zero if none is available). Only interior gaps are filled; boundary extrapolation is absent.
...
For $V$ videos with $n_v$ clips each, source-balanced $R^2$ uses weights $w_{vi}=1/(Vn_v)$ and weighted test mean $\bar y_w$:

$$R_w^2=1-\frac{\sum_{vi}w_{vi}(y_{vi}-\hat y_{vi})^2}{\sum_{vi}w_{vi}(y_{vi}-\bar y_w)^2}.$$

The test mean defines this metric's denominator, not a deployable fitted baseline. Outer test folds contain 19, 19, 19, 18 and 18 videos and 189, 182, 72, 77 and 105 clips. Retained audits check archive IDs/hashes and source-split adherence, not cross-upload content deduplication. Missing person IDs and acquisition/version records limit contamination checks and cohort reconstruction. These splits do not establish generalization under distribution shift.
...
These exploratory percentile intervals use bootstrap seed 812, retain all five fitted seeds and have no familywise or development-selection adjustment. The numerical supplement preserves seed scores and recorded intervals from the real-data grid. The saved audit records pooled-score and bootstrap recomputation from predictions. This revision verifies aggregate consistency; raw predictions and checkpoints are unavailable here, preventing inference or bootstrap reruns.

Reflection augmentation (mirror probability 0.5) has real-data summary evidence from a separate recipe: augmentation reduces normalized teacher-token reflection error after swapping joint positions with feature channels unchanged by $\Delta q=-0.008$ $[-0.010,-0.007]$, while its readout difference is $\Delta R^2=0.004$ $[-0.006,0.013]$. This comparison uses a scheduled teacher EMA, four inner folds and compact features; it is kept separate from the motion grid. An explicit reflection loss comparing aligned online features has synthetic evidence only and requires two additional full encoder passes. Likewise, a readout forced to take the form $g(x)=[h(x)-h(Mx)]/2$ reverses sign algebraically without establishing learned movement information.

Clinical gait asymmetry motivates the observable [8], but GAVD annotations identify neither affected side nor treatment response. Uncalibrated coordinates and absent clinical reference measurements preclude a clinical interpretation of $y$. Public annotation access does not authorize redistribution of separately hosted videos or derived poses; neither is included in the supplement.
