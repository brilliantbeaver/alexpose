...
A representation of an evolving system should support estimates of observable movement as well as its training task. We examine this requirement in gait using a small skeleton Joint-Embedding Predictive Architecture (JEPA). The model predicts hidden skeleton features; a separate regression estimates a signed contrast between left and right movement speeds from its frozen encoder. In source-video cross-validation on 625 clips from 93 videos, trained predictors more consistently match hidden features from the correct clip than from another video. Yet with an expanded feature summary, mean source-balanced $R^2$ for the movement estimate falls from 0.22 at initialization to 0.10–0.11 after training. Motion-weighted and connected-region masks do not establish an improvement over their own random references. This focused negative finding connects latent prediction to evaluation of observable movement: success on hidden-feature matching did not improve the specified movement estimate under the tested training and readout procedure. Future-only prediction remains a separate test.

## Introduction

Walking is an evolving system: coupled body landmarks change position over time, and movement depends on their trajectories. Gait rhythm and coordination motivate temporal modeling, although video-derived coordinates describe only part of the physical process. Temporal world modeling aims to represent such a system well enough to predict its observable behavior. A practical evaluation question is whether a representation trained through prediction preserves information needed for a specified movement measurement.

We study that question with S-JEPA [@sjepa], using a left–right contrast of median coordinate speeds. Clinical gait asymmetry motivates the setting [@stroke]; our contrast has no clinical validation. Our central null is that JEPA training provides no improvement over the same encoder at initialization when a frozen readout estimates this contrast on held-out source videos. The current expanded-readout and mask comparisons are exploratory analyses on a repeatedly examined cohort; they do not retrospectively become registered tests.

The contribution is a domain-specific evaluation of predictive representations, relevant to the reliability of temporal foundation models. The present study establishes neither foundation-model scale nor a gait simulator.
...
Table 1: trained teacher versus matched initialization; exploratory paired intervals conditional on fitted models. Differences precede rounding.
...
The finding is conditional on this cohort, recipe and readout. Forecasting and simulation require tests of future behavior beyond the whole-clip contrast.

## Related work

Skeleton action recognition motivates learned feature targets [@sjepa], motion targets [@mamp] and connected-body masking [@slim]. We vary masks within one JEPA objective without reproducing these complete methods. Video anticipation and planning [@vjepa2], together with transformation-sensitive representation learning [@seqjepa], show why evaluation depends on the intended behavior. Here the endpoint is an observed gait quantity: comparing its recovery before and after training complements those task evaluations.
