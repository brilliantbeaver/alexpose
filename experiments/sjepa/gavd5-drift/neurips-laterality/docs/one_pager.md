...
- We retained 625 pose sequences from 93 GAVD source videos and represented them as moving stick-figure "skeletons" (just dots for shoulders, knees, ankles, and so on). Crucially, we never told the encoder anything about left versus right — it only tried to predict hidden parts of the movement, the way you might guess a covered-up part of a picture.
...
- We were careful about cheating. Across five rotating folds, each model trained on 74 or 75 source videos (436–553 sequences) and tested on 18 or 19 different source videos (72–189 sequences). Every source video was tested exactly once, and all sequences from one source stayed together. We repeated the whole thing with different random starting points and two training variants (50 separately trained models in total). The exact fold counts are recorded in [the canonical split reference](TRAIN_TEST_SPLIT.md). And, importantly, we wrote down all our pass/fail rules *before* looking at any results, so we could not move the goalposts afterward.
