# Feature Prediction and Gait Laterality: A Geometry Audit of Skeleton JEPA

Anonymous working manuscript · Revision 5 · 9 September 2026
...
![Skeleton JEPA training with anatomical identities and masks, an EMA teacher, a source-separated frozen readout, and a separately marked synthetic reflection-loss extension.](../figures/training_pipeline_compact.svg)

*Figure 1. Where laterality enters training. The supplied landmark schema distinguishes anatomical sides; gait-based masking and pooling use that information. The dotted extension adds an explicit original/reflected token penalty to the online encoder, with synthetic evidence only. The signed target enters the downstream readout after encoder training. Clip-local preparation precedes source-fold construction in the implementation and fits no cross-source statistics. All subsequent training draws and views inherit their source assignment. The detailed tensor and gradient diagram is provided in [the expanded figure](../figures/training_pipeline.svg).*
...
### 4.1 Does the prepared input preserve the measurement?
...
### 4.2 Does the original representation satisfy the reflection audit?
...
### 4.3 Does anatomical target selection improve learning?
...
### 4.4 Does targeting motion or connected anatomy help?
...
![Latest paired results: initial and trained laterality readouts, correct-clip predictor correspondence, and alternative-mask confidence intervals.](../figures/learning_results.svg)

*Figure 2. The latest completed grid combines a positive correspondence diagnostic with unfavorable laterality readouts. Large points show mean pooled seed scores; smaller points show seed variation, not confidence intervals. The 375 trained diagnostic rows represent 125 encoder fits under three evaluation masks and repeatedly use the same videos. Initial controls are deduplicated across experiment families. Mask contrasts use each family's count-matched random reference and the retained 2,000-resample source intervals. These intervals condition on fitted models. The new exploratory trained-minus-initial intervals are reported separately in Appendix A.*

### 4.5 What does successful feature prediction establish?
...
## 7. Related work and the remaining empirical question

Masked feature prediction in [I-JEPA](https://arxiv.org/abs/2301.08243) and [S-JEPA](https://www.ecva.net/papers/eccv_2024/papers_ECCV/papers/04755.pdf) motivates the model family. [MAMP](https://arxiv.org/abs/2308.07092) motivates motion-informed targets and sampling, while [SLiM](https://arxiv.org/abs/2603.10648) provides recent connected-region masking precedent. Our MAMP-style arm adapts its official-code sampling convention while retaining the JEPA feature objective; it is not a reproduction of MAMP's complete motion-prediction method.

[seq-JEPA](https://proceedings.neurips.cc/paper_files/paper/2025/hash/2f63d2963526bdd9ff1b8bcc2dc9905a-Abstract-Conference.html) studies invariant and equivariant representations. [Soft Equivariance Regularization](https://proceedings.iclr.cc/paper_files/paper/2026/hash/3be6511c8f56d0dca4b5ed59fdf9b2f4-Abstract-Conference.html) examines geometric regularization and its tradeoffs. Our separation of a geometric score from predictive utility belongs in that established context.

Recent preprints make the next comparison more specific. [V-JEPA 2.1](https://arxiv.org/abs/2603.14482) studies dense and multilayer predictive supervision; [Human-JEPA](https://arxiv.org/abs/2608.21160) examines human-focused perception and anticipation. [GaitJEPA](https://github.com/AVAuco/GaitJEPA) already applies predictive learning to gait silhouettes. These works limit any claim of novelty based on combining gait with JEPA. They motivate evaluating what a predictor adds beyond its encoder on a shared movement endpoint, without predicting a gain on this small pose dataset.
...
| Future horizon | Observed-future feature RMSE | Predicted-future feature RMSE |
|:--|--:|--:|
