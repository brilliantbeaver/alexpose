...
Paper mode reconciles the official annotation inventory to the local derived-pose cache before QC. Counts, privacy-preserving inventory digests, pose-model identity, visibility threshold, and the scientific extraction-generation census are frozen in `config/protocol.json`; added, removed, or provenance-mismatched archives fail closed. Cache paths are portable provenance strings: validation compares their terminal condition and annotation filename using both POSIX and Windows separators and never dereferences the extraction host's absolute path.

A metadata-only split-provenance refresh may label an archive with the `gavd5_pose_v3_split_provenance` schema. This does not claim that its pose coordinates were re-extracted. For such archives, the loader requires `cache_origin_version` and uses that preserved value for the locked extraction-generation census and nuisance control. Audits report the archive-schema census separately so neither layer is silently conflated. The attrition report therefore starts at 666 annotation files, records the 642 available pose archives, and then applies the locked pose/target QC rules. The suite never reads, displays, or redistributes raw videos, public URLs, or identity-bearing frames.

The ordinary `cohort_digest` remains container-sensitive and therefore changes when archive metadata is repacked. Reuse across such a transition is denied by default. A one-time compatibility exception must name the exact old cohort digest, new cohort digest, protocol digest, and split digest in `laterality/cohort_compatibility.json`, and it is accepted only when the current cohort also matches the reviewed scientific-content digest over every accepted identity, count, target, coverage value, model coordinate, validity mask, target-pair contrast, extraction generation, and missingness feature. This narrow gate permits metadata-only migrations without weakening rejection of scientific input drift.
