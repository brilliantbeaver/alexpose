...
## Codex AR-5 (2026-08-04) — clean-room, committed tree — archived artifacts/reviews/AR5_cleanroom_codex_2026-08-04.md
Codex `gpt-5.6-sol`, `codex review --base main` on the COMMITTED branch (SHA eed5166), reviewing the full correctness program (~14k lines, both branch commits). Four findings, all reproduced against the code by the lead and FIXED this session. None invalidates the frozen R1 macro-F1 ordering, but two are real objective/metric defects.
- **P1-A centering EMA applied per-example, not per-batch** (`train_v2.py:210-216` + `losses.py`) — CONFIRMED. The loop called `ce(...)` once per example and `forward` updated the center each call, so with batch B the beta=0.9 EMA fired B times per step (effective retention ~0.9^B) and the loss was order-dependent within a batch. **FIXED:** `CenteringSharpeningCE.forward(..., update_center=False)` + new `update_center_from(all_targets)`; the loop now scores every example against one center snapshot and updates the center once from all masked target tokens. New regression `test_centering_ema_updates_once_per_batch` (center == one EMA step). NOTE: the frozen R1 (0.438) was produced with the old per-example centering; it is NOT re-run here. Re-running R1 on the fixed objective is an explicit, authorized future step, not a silent overwrite.
- **P1-B controls reported fold-mean macro-F1 while R1 reports pooled** (`scripts_phase0_provenance.py:186-218`) — CONFIRMED. Macro-F1 is nonlinear so fold-mean != pooled; nb_06 + docs put them in one scoreboard. **FIXED:** `run_control` now also computes `pooled_macro_f1` from saved per-clip OOF; `E0_results.json` augmented with pooled control numbers (fold-means verified byte-identical, so the LOCKED registry/features are unchanged — Phase 0 was NOT re-run and the fold_registry is untouched). nb_06 scoreboard + docs 06/07 now cite pooled controls. Corrected ordering (all pooled): mean_std_pose 0.703 > RF 0.667 > mean_pose 0.657 > visibility 0.636 > duration 0.469 > body_proportion 0.440 > **S-JEPA 0.438** > chance. Headline conclusion (S-JEPA below RF and strong controls) holds.
- **P2-C source_exposure counted stored windows, not actual draws** (`train_v2.py:241-244`) — CONFIRMED (diagnostic-only). **FIXED:** exposure now accumulated from the indices `get_batch()` actually draws each step, so it reflects budget/seed/replacement sampling. (On resume it counts the current run's draws.)
- **P2-D mask "free a token" fallback could free a non-clinical token** (`masking_v2.py:132-135`) — CONFIRMED (edge case). Module documents "at least one lower-body/contralateral cue stays visible"; the fallback freed an arbitrary token. **FIXED:** when all clinical tokens are targeted, free a random *clinical* token as context. New regression `test_mask_keeps_a_clinical_context_cue`.
- Post-fix validation: `py_compile` OK; **17 tests pass** (15 prior + 2 new AR-5 regressions; D3 no-leak and D4 save/resume still pass under the centering change); all 7 notebooks idempotent; nb_03->04->06 smoke-run clean; pooled scoreboard renders the corrected ordering. Codex could not run the suite (sandbox); verified by the lead on the pinned 3.12 venv. AR-5 dispositions closed; zero unresolved P1.
